...
**サイト:** https://namo40.github.io/tech-study/ja/

> [!NOTE]
> このプロジェクトは AI（Claude）と共同で制作しました。
...
ページはシーンを自分で持つか、`sceneStep` で親シーンの 1 つのステップを指すか、親シーンの全体をエイリアスとして借ります。後ろの 2 つは親のステップ本文をそのまま使います。

ページには 1～10 の難易度 `level` も持たせられます。タグと同じく英語のファイルにだけ書き、ほかの言語はその値をそのまま読みます。ページではカテゴリーの横にチップとして示し、索引では 10 段階を 5 つの区間にまとめて絞り込みます。
...
シーンはページを開くと自動的に再生されます。ステップボタン、前後のボタン、矢印キーでステップを選ぶと、そのステップの先頭に移って停止するので図を確かめられ、再生を押すとそこから続きます。読む人がモーションの抑制を選んでいる場合は、プレーヤーは自動では再生を始めず、最初のフレームで止まったまま待ちます。現在のステップは、広い画面ではステージの横に、狭い画面ではステージの上に説明カードとして表示するので、説明と図が互いを隠すことなくいっしょに見られます。字幕は絵ではなくページのテキストであり、状態は色だけでなく文字でも示し、結果はチェックとバツの記号で区別するので、色だけに頼る場面はありません。ヘッダーの文字サイズボタンを押すと本文と字幕が 5 段階で大きくなり、選んだサイズは保存されます。ステージに描かれる図は設計どおりの大きさのままです。
