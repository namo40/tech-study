...
steps:
  - title: "Two writes, no transaction"
    text: "Commit the order, then publish the event, and the broker is down: the order is in its table and the world never hears of it. Publish first instead, and a failed commit invents an event for an order that does not exist."
...
- Publishing in order costs throughput, and insert order is not commit order. Two concurrent transactions can commit the lower id second, so a relay that walks forward on "id greater than the last one I sent" skips that row for good; claim pending rows by a status column or a lease instead. If consumers care about the order of events for one order or one account, serialize inside that key and let the relay batch everything else.
