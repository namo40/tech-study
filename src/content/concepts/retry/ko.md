...
steps:
  - title: "Retry"
    text: "호출이 한 번 실패합니다. Retry가 잠깐 기다렸다가 다시 보내고, 이번에는 성공합니다. 그 실패가 일시적이었다는 데 거는 셈입니다."
  - title: "지수 백오프와 Jitter"
    text: "기다리는 시간이 매번 길어지고, 무작위 편차를 더해 재시도가 같은 순간에 몰리지 않게 합니다. 시간을 두 배씩 늘려 의존 서비스에 회복할 틈을 줍니다."
  - title: "Retry Storm"
    text: "같은 박자로 재시도하는 클라이언트들은 회복 중인 Service를 파도처럼 때려 다시 쓰러뜨립니다. Jitter가 그 파도를 흩어 놓습니다."
  - title: "한계가 있습니다"
    text: "재시도는 예산이나 Deadline에서 멈추고, 반복하면 결과가 달라지는 호출에는 쓰지 않습니다. 그다음은 빠르게 실패하고 Circuit Breaker에 넘깁니다."
related:
...
- 장애가 일시적일 때. 연결 끊김, DNS 오류, 408, 429, 503, 락 대기 초과가 여기에 해당합니다.
...
- 재시도는 한 계층만 담당하게 합니다. 클라이언트, 게이트웨이, 서비스가 각각 3회씩 재시도하면 실패 하나가 호출 27번이 됩니다.
...
기본 `ShouldHandle`은 5xx 응답, 408, 429, `HttpRequestException`, 시도별 Timeout을 일시 오류로 봅니다. `AddStandardResilienceHandler()`는 같은 Retry 전략을 rate limiter, Timeout, Circuit Breaker와 함께 묶어 제공하므로, 기본값이 맞지 않을 때만 파이프라인을 직접 구성하면 됩니다.
