...
.NET에서는 이 작업이 `BackgroundService` 안에서 돕니다. 오래 사는 `IHostedService`를 위한 기반 클래스입니다. 웹 애플리케이션에 그대로 등록하는 것이 가장 작은 한 걸음이고, 가볍고 잃어도 되는 작업이라면 그 선택이 맞는데, 그때 작업은 사이트와 모든 것을, 곧 프로세스와 메모리와 배포와 스케일링 규칙을 공유하기 때문입니다. Worker Service 프로젝트는 같은 클래스를 자기 호스트 안에 두고, 따로 배포하며, 요청 수가 아니라 큐 길이를 보고 늘립니다. 그렇게 갈라놓는 것이 이 패턴의 요점이고, 작업이 요청과 CPU를 두고 다툴 만큼 무거워지는 순간 옮길 만합니다.
