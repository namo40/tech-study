...
- Register the loader per request, which is what the Hot Chocolate integration does for you. The batch method also gets a service scope of its own, so the `ShopDbContext` above is not the instance the resolvers are using — which is what makes injecting a scoped, deliberately not thread-safe `DbContext` here safe, and what you give up by handing the loader a context from outside it. If you write the same pattern by hand for a REST or gRPC composition layer, keep the same lifetime: a scoped service holding a dictionary and a batching channel, disposed with the request, is the whole of the pattern.
