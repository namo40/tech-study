...
steps:
  - title: "주장은 공짜이고, 인증은 증거 검사입니다"
    text: "이름 하나 대는 데는 아무 비용이 들지 않습니다. 고스트는 이름만 묻는 문을 그대로 지나갑니다. 진짜 인증은 증거를 요구하고 그 자리에서 검증합니다. 아는 것, 가진 것, 그 자신인 것입니다. 문은 검사하는 순간부터 장식이기를 멈춥니다."
  - title: "증거는 훔칠 수 있고, 시스템은 가장 약한 증거만큼만 강합니다"
    text: "옳은 비밀번호는 잘못된 손에서도 같은 검사를 통과합니다. 검증자는 의도가 아니라 증거를 보기 때문입니다. 그래서 비밀번호는 홀로 다니지 않고 방어를 두른 채 다닙니다. 검증은 금고가 아니라 문입니다."
  - title: "기계에도 신원이 있고, 비밀번호를 칠 손가락은 없습니다"
    text: "보여 줄 것이 없으면 서비스도 다른 누구처럼 거절당합니다. 한 번 발급받아 호출마다 제시하고 등록부와 대조되는 키가 있으면 통과합니다. 소지가 곧 증거이고, 그래서 새어 나간 키는 곧 신원 유출입니다."
...
- 비밀번호를 되돌릴 수 있는 형태로 저장하지 않습니다. 암호화는 키를 손에 넣은 사람이면 누구나 되돌릴 수 있고, 그 키는 데이터 옆에 있습니다. 솔트를 넣은 적응형 해시를 씁니다. PBKDF2, bcrypt, scrypt, Argon2 중 하나를, 비밀번호 하나를 검증하는 비용은 서버에 저렴하고 10억 개를 시도하는 비용은 공격자에게 저렴하지 않도록 조정해서 씁니다. 직접 방식을 발명하지 말고 프레임워크가 제공하는 것을 씁니다.
- 실패에 속도 제한과 잠금을 걸지 않으면 검증자가 오라클(정답을 알려 주는 장치)이 됩니다. 즉시, 그리고 영원히 답해 주는 문은 수백만 번의 추측도 기꺼이 확인해 줍니다. 게다가 "그런 사용자 없음"과 "비밀번호 틀림"을 구분해서 답하면 계정 목록부터 털립니다. 실패가 몇 번 쌓이면 느려지게 하고, 두 경우에 같은 답을 주고, 무슨 일이 있었는지 기록합니다.
...
- API key는 신원이므로 신원답게 다룹니다. 키마다 필요한 만큼으로 범위를 좁히고, 저장소에 커밋하는 설정이 아니라 비밀로 보관하고, URL과 로그에서 빼고, 감당할 수 있는 주기로 교체합니다. 새어 나간 키는 새어 나간 비밀번호가 아니라, 알아차릴 사람이 없는 새어 나간 계정입니다.
- 토큰 형식을 직접 만들지 않고, 남이 만든 것의 검증도 건너뛰지 않습니다. 서명은 검증이 하는 만큼만 값어치가 있습니다. 알고리즘, 발급자, 대상, 만료를 매번 확인하고, 토큰이 스스로 제안하는 알고리즘은 절대 받아들이지 않습니다.
...
ASP.NET Core는 인증을 스킴 단위로 나눕니다. 스킴은 요청에서 증거를 읽어 `ClaimsPrincipal`로 바꾸는 법을 아는 이름 붙은 핸들러이고, 기본 스킴은 아무것도 지정하지 않았을 때 실행되는 것입니다. 브라우저에는 쿠키, API에는 bearer token이 보통의 두 경우이며 한 애플리케이션이 둘 다 가질 수 있습니다. 다만 기본 스킴은 하나뿐이므로, API 엔드포인트는 다른 쪽 이름을 직접 지정해야 합니다. `[Authorize(AuthenticationSchemes = "api")]`를 붙이거나 `AddAuthenticationSchemes("api")`로 만든 정책을 씁니다.
...
builder.Services.AddAuthentication(CookieAuthenticationDefaults.AuthenticationScheme)
    .AddCookie(options =>
    {
        ...
        // 절대 만료 8시간입니다. 활동해도 연장되지 않으므로 모든 세션에는
        // 누군가 정한 끝이 있습니다. 슬라이딩 만료도 합리적인 다른 답이며,
        // Cookie Authentication 페이지가 그쪽을 권합니다.
...
// 잠금은 호출자에게 알리지 않습니다. 다른 답을 주면 어느 계정이 존재하는지
// 확인해 주는 셈입니다. 실제 계정만 잠길 수 있기 때문입니다. 로그에 남기고,
// 경보를 걸고, 비밀번호가 틀렸을 때와 똑같이 답합니다.
if (result.IsLockedOut) logger.LogWarning("Lockout on {UserName}", userName);
if (!result.Succeeded) return Results.Unauthorized();
```

잠금은 코드가 아니라 설정이고, 문과 오라클을 가르는 것이 바로 이것입니다.
...
기계 쪽에서는 인증서 인증도 다른 것과 같은 스킴입니다. 핸드셰이크는 요청이 파이프라인에 닿기 전에 서버에서 끝나므로, 핸들러는 이미 도착한 인증서의 체인과 폐기 상태를 검증한 뒤 `OnCertificateValidated`에서 우리만 답할 수 있는 질문을 던집니다. 이 정상 인증서가 우리가 등록한 호출자의 것인가입니다. 애초에 인증서를 요구하는 것은 Kestrel의 몫이고, 그것은 스킴 옵션이 아니라 전송 설정입니다. 그쪽은 Mutual TLS 페이지에 있습니다.
...
가장 좋은 비밀은 존재하지 않는 비밀입니다. Azure에서는 Managed Identity가 실행 중인 워크로드에 플랫폼이 발급하고 교체하는 자격 증명을 주므로, 연결 문자열에는 호스트 이름만 남습니다.

```csharp
// 설정에도 키가 없고, 볼트에도 키가 없고, 교체할 키도 없습니다.
