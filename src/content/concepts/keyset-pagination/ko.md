...
정렬 키는 고유해야 합니다. 그렇지 않으면 시크가 동점 행 무리 한가운데를 가르고 지나가면서 행이 사라집니다. `CreatedAt` 하나로만 정렬하면 페이지 경계와 같은 시각을 가진 행이 전부 위험해집니다. 해결은 기본 키를 뒤에 붙여 전순서로 만드는 것이고, 그러면 비교도 조건 두 개가 아니라 두 컬럼을 하나로 다루는 비교여야 합니다. 손으로 쓰는 SQL에서는 행 값(row value)을 지원하는 데이터베이스라면(PostgreSQL, MySQL, SQLite는 지원하고 SQL Server는 지원하지 않습니다) `WHERE (CreatedAt, Id) > (@lastCreated, @lastId)`입니다. LINQ에서는 `.Where(o => o.CreatedAt > last.CreatedAt || (o.CreatedAt == last.CreatedAt && o.Id > last.Id))`가 되는데, 길기는 해도 경계의 양쪽에서 모두 맞는 유일한 형태입니다. 내림차순은 모든 비교를 뒤집으므로, 영리한 쿼리 하나로 합치지 말고 두 방향을 각각 쓰고 각각 테스트합니다.

인덱스는 정렬과 정확히 같아야 하고 방향까지 같아야 합니다. 그러지 않으면 시크가 조용히 테이블 전체 정렬로 바뀌어 아무것도 얻지 못합니다. `(CreatedAt, Id)`로 정렬하면 인덱스도 `(CreatedAt, Id)`입니다. 덧붙일 것이 둘 있습니다. keyset은 삽입과 삭제에 흔들리지 않으므로, 살아 있는 테이블을 넘기는 독자는 각 행을 많아야 한 번 봅니다. 느리지 않더라도 내보내기와 동기화 작업이 keyset을 써야 하는 이유가 그것입니다. 그리고 키가 무언가를 흘린다면 날것 그대로 클라이언트에게 주지 않습니다. 같은 값을 담은 불투명한 서명 cursor는 겉모습만 나은 같은 구조이고, 그것이 바로 cursor 페이지네이션입니다.
