---
title: "Redis"
summary: "Redis는 인스턴스들이 함께 쓰는 인메모리 데이터 구조 서버입니다. 문자열과 해시, 정렬 집합, 스트림을 들고 있는 빠른 프로세스 하나가 분산 캐시와 세션 저장소, 속도 제한 카운터, 분산 락, 가벼운 큐의 답이 되는 이유가 여기 있습니다."
category: "캐시"
related:
  - label: Cache-Aside
    slug: cache-aside
...
- 캐시가 프로세스마다가 아니라 공유되어야 할 때 꺼냅니다. 인메모리 캐시는 인스턴스 수만큼 곱해집니다. 복제본 열 대는 사본 열 개를 들고, 열 번 예열하고, 각자 따로 만료됩니다. `IDistributedCache` 뒤의 Redis 하나면 모든 복제본이 같은 항목을 보고, HybridCache를 등록했을 때 승격되는 L2도 이것입니다.
...
- 더 나은 자리가 없을 때 협조용 기본 요소를 여기에 둡니다. 분산 락과 속도 제한 카운터, 중복 방지 기록은 모두 모든 인스턴스가 동의하는 한 곳을 필요로 합니다. 명령이 단일 스레드로 원자적으로 처리되므로 증가시키고 비교하는 모양을 정확히 만들기가 쉽습니다.
...
- 세션과 데이터 보호 저장소도 같은 방식으로 붙습니다. `AddStackExchangeRedisCache`에 `AddSession`을 더하면 세션 상태가 인스턴스 밖으로 나가고, 같은 연결이 데이터 보호 키 링도 들 수 있습니다. 스케일 아웃한 애플리케이션이 두 번째로 공유해야 하는 것이 보통 이것입니다.
- 관리형 서비스는 운영 이야기를 바꾸지 API를 바꾸지 않습니다. Azure Managed Redis는 패치와 장애 조치와 TLS를 맡아 주므로 애플리케이션은 같은 클라이언트로 같은 프로토콜을 그대로 씁니다. 일이 옮겨 가는 곳은 크기 산정과 축출 정책, 그리고 영속화가 정말 필요한지 정하는 쪽입니다.
