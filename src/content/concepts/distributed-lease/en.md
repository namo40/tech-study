...
That makes the TTL a real design decision rather than a default to copy. Too short and a holder that is merely slow loses the key in the middle of its work. Too long and a crashed holder blocks everyone for the whole window. The usual rule is to set the TTL comfortably above the p99 duration of the critical section, then renew at a fraction of it, so that several renewals can fail before the lease is actually at risk. In the scene the TTL is drawn as an arc that drains and a renewal refills it; renewing at sixty percent of the window means two ticks have to be lost before the key is in danger.
