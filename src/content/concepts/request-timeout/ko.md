...
summary: "request timeout은 호출자가 원격 호출에 두는 상한입니다. 상한이 없으면 멈춘 의존 서비스가 호출자를 영원히 붙잡고, 상한이 있으면 호출자는 빠르게 실패하고 남은 예산을 아래로 넘기며 더는 필요 없는 작업을 취소합니다."
category: "복원력과 장애 대응"
scene: request-timeout
steps:
  - title: "타임아웃이 없으면"
    text: "의존 서비스가 멈추면 호출도 멈추고, 그 뒤의 스레드와 연결도 함께 멈춥니다. 사용자는 5초 뒤에 포기하지만 서버는 여전히 기다리고 있습니다."
  - title: "상한은 여러 개입니다"
    text: "연결, 한 번의 시도, 요청 전체는 서로 다른 세 가지 제한입니다. 각각을 정해 두면 느린 응답이 빠른 오류가 되고 스레드는 다시 일하러 돌아갑니다."
  - title: "남은 예산을 아래로 넘깁니다"
    text: "요청 하나에 800ms입니다. 데이터베이스가 300을 썼으니 다음 호출은 자기만의 새 타임아웃이 아니라 남은 500을 받습니다. 예산이 안쪽으로 포개지지 않고 더해지면 deadline을 넘깁니다."
  - title: "더는 기다리지 않는 일은 취소합니다"
    text: "취소 없는 타임아웃은 의존 서비스에 아무도 받지 않을 ghost work를 남깁니다. CancellationToken을 끝까지 흘려보내야 끊음이 실제가 됩니다."
related:
  - label: Timeout
    slug: timeout
  - label: Deadline
...
- 여러 의존 서비스로 갈라지는 요청마다. 요청 하나에 deadline 하나를 주고, 각 호출의 타임아웃은 남은 예산에서 끌어냅니다.
...
- 모든 비동기 호출에 `CancellationToken`을 넘깁니다. 취소하지 않는 타임아웃은 낭비를 호출자에서 의존 서비스로 옮길 뿐입니다.
- 너무 짧은 타임아웃은 평범한 변동만으로도 건강한 호출을 실패시킵니다. 정상 상태 의존 서비스의 p99에서 시작해 여유를 더합니다.
