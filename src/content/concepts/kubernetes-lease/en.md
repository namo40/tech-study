...
One property is worth stating plainly because it surprises people who expect a lock service. Nothing on the server side expires a Lease. The object does not disappear when `renewTime` plus `leaseDurationSeconds` has passed, and the API server will not refuse a write from a holder whose claim went stale. Expiry is a judgement made by the candidates: each one reads the object and compares the moment it last saw the record change, plus `leaseDurationSeconds`, against its own clock, and only attempts to take over when the holder looks overdue. Timing the window from the observation rather than from the `renewTime` the holder wrote is deliberate, because it keeps the decision out of the gap between the writer's clock and the reader's. That is why the same clock caveats matter more here rather than less, and why the leader's own safety cannot rest on its in-memory belief that it still holds the object.
