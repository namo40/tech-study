---
title: "Bulkhead"
summary: "Bulkhead는 의존 서비스마다 슬롯 풀을 따로 줍니다. 느려지거나 실패하는 의존 서비스는 자기 칸만 채울 수 있고, 서비스의 나머지는 계속 동작합니다."
category: "복원력과 장애 대응"
scene: bulkhead
steps:
  - title: "공유 풀 하나"
    text: "A로 가든 B로 가든 모든 호출이 같은 풀에서 슬롯을 가져갑니다. 두 의존 서비스가 건강할 때는 문제가 없습니다."
  - title: "Bulkhead가 없으면"
  ...
  - title: "타임아웃이 슬롯을 되돌립니다"
    text: "멈춘 호출이 슬롯을 영원히 쥐고 있으면 안 됩니다. 칸마다 Timeout을 함께 두고 그 의존 서비스의 예산에 맞게 크기를 정하면, B는 다른 누구도 모르게 회복합니다."
related:
...
- 서비스 하나가 여러 의존 서비스를 호출하고, 그중 느린 하나가 나머지까지 끌어내리면 안 될 때
- 호출자나 테넌트에 따라 중요도가 달라 일정 용량을 미리 확보해 두어야 할 때
- 의존 서비스가 빠르게 실패하기보다 느려지는 방식으로 나빠진다는 것을 알고 있을 때

## 주의점

- Timeout 없는 Bulkhead는 문제를 미루기만 합니다. 멈춘 호출은 Timeout이 풀어 줄 때까지 슬롯을 쥐고 있습니다.
...
의존 서비스마다 `HttpClient`와 파이프라인을 따로 둡니다.
