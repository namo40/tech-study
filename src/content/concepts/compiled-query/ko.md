...
- 시작 시간에 민감한 프로세스를 겨냥할 때 씁니다. 쿼리 준비를 명시적이고 정적으로 뿌리내린 대리자로 옮겨 두면 작업이 예측 가능해지고 첫 요청 경로에서 빠집니다. 일주일째 데워진 서버보다 수명이 짧은 함수 인스턴스에 더 중요한 이야기입니다. Native AOT는 이것과 다른 메커니즘입니다. EF Core는 `dotnet ef dbcontext optimize --precompile-queries`가 빌드 시점에 생성하는 미리 컴파일된 쿼리로 거기에 이르는데, 이 기능은 아직 실험 단계입니다.

## 주의점

- 먼저 측정합니다. EF Core는 이미 쿼리 번역을 캐시하기 때문입니다. LINQ 쿼리는 처음 실행될 때 번역되어 EF Core의 쿼리 캐시에 담기고(같은 옵션으로 만든 컨텍스트는 모두 이 캐시를 공유합니다), 이후 실행은 식 트리를 알아보고 그 번역을 재사용합니다. 컴파일된 쿼리가 없애 주는 것은 번역이 아니라 알아보기와 캐시 조회이므로, 이득은 호출당 수 마이크로초대입니다. 분당 십만 번 지나가는 핫 패스에서는 가질 만한 값이고 그 밖의 어디에서도 가치가 없습니다.
- 나오는 SQL은 동일하고, 그래서 느린 쿼리는 정확히 그대로 느립니다. 문장에 인덱스가 없거나 컬럼을 너무 많이 돌려주거나 반복문 안에서 행마다 실행되고 있다면, 컴파일해도 데이터베이스가 받는 것은 같은 문장과 같은 실행 계획입니다. 모양을 먼저 고칩니다. 반복문 이야기는 N+1 Query 페이지에, 나머지는 Query Plan 페이지에 있고, 어느 쪽 처방이든 여기서 얻는 것보다 몇 자릿수 더 큰 값어치를 합니다.
- `DbContext`와 모든 매개변수는 람다의 인자여야 하고 캡처한 변수여서는 안 됩니다. 캡처한 값은 컴파일 시점에 식 안에 고정되므로, 대리자는 만들어질 때 마침 범위 안에 있던 그 아이디의 결과를 계속 돌려주게 됩니다. 캡처한 컨텍스트는 두 번째 요청에서 이미 해제된 컨텍스트입니다. 인자로 넘기는 방식은 대리자가 자기 상태를 들고 있지 않게 만들어, 동시에 들어오는 요청들이 하나의 대리자를 함께 써도 안전하게 해 주기도 합니다.
...
```csharp
// 쿼리마다 정적 대리자 하나, 프로세스 수명 동안 한 번만 만듭니다.
...
// 시퀀스 결과는 IAsyncEnumerable로 오므로 CancellationToken 매개변수가 없습니다.
...
// 컨텍스트와 매개변수는 인자입니다. 둘 중 하나라도 캡처하면 그것이 버그입니다.
var product = await GetProductById(db, id, ct);
```

- `EF.CompileQuery`는 동기 쪽 쌍둥이이고 규칙은 같습니다. 주변 코드가 정말로 동기일 때만 고릅니다. 요청 스레드에서 데이터베이스 호출을 붙들고 기다리는 값은 절약한 컴파일보다 훨씬 큽니다.
...
- 컴파일된 쿼리와 prepared statement 재사용은 같은 여정의 서로 다른 절반을 풉니다. 이쪽은 명령이 전송되기 전 .NET 프로세스 안의 일을 줄이고, 매개변수화된 명령과 서버의 계획 캐시는 명령이 도착한 뒤의 일을 줄입니다. 어느 쪽도 다른 쪽을 대신하지 못합니다.
