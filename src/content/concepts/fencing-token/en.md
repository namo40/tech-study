...
Implementing it costs one column and one predicate. In a relational database it is `WHERE last_token < @token` on the update, which is the same shape as an optimistic concurrency check but keyed on the lease rather than on the row's own version. In a document store it is a conditional write on the same field. In an object store it is a read of the current token followed by a write guarded by an `If-Match` precondition, since the store can only compare for equality rather than for order. The pattern only breaks down when the resource cannot express a condition at all, and that is worth knowing before the design depends on a lock: if the destination cannot refuse a stale write, nothing downstream of it can either.
