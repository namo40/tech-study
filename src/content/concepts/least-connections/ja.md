...
- 長く保たれる接続。ロードバランサーは接続を一度ルーティングするだけで、その後はその接続が好きなだけトラフィックを運びます。接続数を数えることが残された唯一の手がかりです。
- 速度の違うバックエンド。ハードウェアが違うからでも 1 台が不調だからでも、遅いサーバーには処理中のリクエストがたまり、その数がこれ以上足すなという合図になります。

## 注意点

- 数えているのは処理中のリクエストであって、待ち行列ではありません。即座に失敗するサーバーはいちばん空いているように見えるので、ヘルスチェックが気づくまで壊れたインスタンスのほうが健全なインスタンスより多くのトラフィックを引き寄せます。シーンの 3 番目のステップがそれで、least connections とヘルスチェックが一緒でなければならない理由です。
...
- 同点には規則が必要です。いつも先頭を取るのではなく、順番を回して解きます。そうしないと空いているプールですべてのリクエストが 1 台に集まります。
- これが揃えるのは同時実行数であってレイテンシではありません。テールが問題なら、サーバーごとの同時実行上限とタイムアウトを組み合わせ、遅いサーバーが仕事を抱え込まずに手放すようにします。
...
builder.Services.AddReverseProxy().LoadFromMemory(
    ...
    [
        ...
        {
            ClusterId = "api",
            // LeastRequests は処理中の正確な件数を読み、PowerOfTwoChoices は
            // 宛先を 2 つ抽出して、そのうち少ないほうを取ります。
...
上の `ActivityTimeout` は飾りではありません。least connections はリクエストがいつか数から抜けることで初めて成り立ち、永遠にぶら下がったリクエストは枠を永遠に占めます。ポリシーとタイムアウトは同じ仕組みを両端から見たものです。Nginx は `least_conn`、HAProxy は `leastconn` と綴り、Envoy の `LEAST_REQUEST` はサンプル数を設定できる power of two choices ですが、クラスターがポリシーを指定しなかったときに Envoy が使うのは `ROUND_ROBIN` です。
