---
title: "Key Ring"
summary: "키 링은 ASP.NET Core Data Protection이 쿠키와 antiforgery 토큰, 수명이 짧은 페이로드를 보호할 때 쓰는 교체되는 키 묶음입니다. 하나는 현재 키이고 예전 키들은 복호화용으로 남으며, 애플리케이션의 모든 인스턴스가 같은 링을 보고 있어야 합니다."
category: "애플리케이션 보안"
related:
  - label: Key Rotation
    slug: key-rotation
...
- 애플리케이션이 로드 밸런서 뒤로 처음 들어가는 순간 의도적으로 설정합니다. 쿠키 인증, antiforgery 토큰, TempData는 전부 보호된 페이로드이고, 한 인스턴스가 보호한 페이로드는 다른 링을 든 인스턴스가 읽지 못합니다. 링을 공유하지 않는 레플리카 두 대는 두 요청 중 한 번은 로그인하지 않은 것처럼 보인다는 뜻입니다.
...
- 컨테이너 기본값은 임시 링이고, 아무도 경고해 주지 않습니다. 지속화를 설정하지 않으면 키는 쓰기 가능한 레이어 안의 디렉터리나 메모리로 갑니다. 재시작마다 사라지고 레플리카끼리 공유되지도 않습니다. 애플리케이션은 잘 시작하고, 아무도 읽지 않는 경고를 남기고, 개발자의 한 대짜리 컴퓨터에서는 완벽하게 동작합니다.
...
- Data Protection은 일시적인 페이로드를 위한 것이지 장기 보관 암호화용이 아닙니다. 만료된 키는 링에 남아 여전히 해제되므로, 옛 페이로드를 읽을 수 없게 만드는 것은 폐기된 키, 지워진 키 파일, 잃어버린 링, 바뀐 애플리케이션 이름입니다. 그중 어느 것도 계획을 세워 둘 수 있는 일정에 따라 일어나지 않습니다. 3년 뒤에도 복호화되어야 하는 필드는 수명 주기를 우리가 직접 관리하는 별도 암호화 체계에 두어야 합니다.
- 교체는 알아서 일어나고, 예전 키가 남아 있는 데는 이유가 있습니다. 기본 수명은 90일이고, 대체 키는 현재 키가 만료되기 며칠 전에 미리 쓰이되 만료되는 시점에야 활성화됩니다. 모든 레플리카가 새 키를 볼 시간을 주는 장치가 이것입니다. 이전 키들은 기존 쿠키가 만료될 때까지 계속 동작하도록 링에 남습니다. 정리한다고 예전 키를 지우는 것이 평범한 교체를 전원 로그아웃으로 바꾸는 행동입니다.

## .NET에서는

- 링을 인스턴스 바깥에 저장하고 그 자리에서 암호화합니다. 이 두 호출이 운영 설정의 전부이고(`Azure.Extensions.AspNetCore.DataProtection.Blobs`와 `Azure.Extensions.AspNetCore.DataProtection.Keys`에서 옵니다), 애플리케이션 이름이 두 번째 애플리케이션도 같은 페이로드를 읽게 해 주는 부분입니다.

```csharp
builder.Services.AddDataProtection()
    // 공유되고 지속됩니다. 모든 레플리카가 같은 링을 읽습니다.
    .PersistKeysToAzureBlobStorage(blobUri, credential)
    // 저장할 때 암호화합니다. 블롭을 읽는 것과 키를 쥐는 것은 다릅니다.
    .ProtectKeysWithAzureKeyVault(keyIdentifier, credential)
    // 보호 바인딩의 일부입니다. 이것을 바꾸면 기존 페이로드가 전부 무효가 됩니다.
...
- `SetDefaultKeyLifetime`은 교체 주기만 바꿉니다. 짧게 잡으면 키가 더 자주 만들어지고, 만료된 키는 전과 똑같이 복호화용으로 남으며, 7일 아래로는 내려가지 않습니다. 그러니 더 엄격한 정책이 필요할 때 안전하게 돌릴 손잡이는 수명이지 키를 손으로 지우는 쪽이 아닙니다.
