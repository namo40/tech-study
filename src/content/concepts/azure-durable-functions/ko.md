...
- 내구성 있는 오케스트레이션은 원하지만 서버는 두고 싶지 않을 때 씁니다. 히스토리는 앱에 설정된 저장소 백엔드에 살고, 스케일 컨트롤러가 큐 깊이를 보고 인스턴스를 늘리고 줄이며, 소비 요금제는 인스턴스가 기다리며 보낸 며칠이 아니라 액티비티 실행에 요금을 매깁니다. 일주일을 놀고 있는 워크플로는 노는 동안 거의 공짜입니다.
- 그림이 사실은 프로그램일 때 상태 머신을 코드로 씁니다. 오케스트레이터 안의 분기와 반복과 오류 처리는 `if`와 `while`과 `try`입니다. 같은 전이를 테이블에 흩어 놓는 것보다 읽기도 테스트하기도 쉽고, 손으로 만든 상태 머신이었다면 따로 설계해야 했을 감사 기록을 히스토리가 그냥 줍니다.

## 주의점

- 오케스트레이터는 결정적이어야 합니다. 재개할 때마다 처음부터 다시 실행되기 때문입니다. `DateTime.UtcNow`, `Guid.NewGuid()`, `Random`, 직접 부르는 HTTP나 데이터베이스 호출, 순서가 보장되지 않는 LINQ 정렬은 모두 재생할 때 처음과 다른 답을 내놓고, 런타임은 자기 자리를 잃습니다. `context.CurrentUtcDateTime`과 `context.NewGuid()`와 `CallActivityAsync`를 쓰고, 첫 오케스트레이터를 쓰기 전에 코드 제약 문서를 읽어 둡니다.
...
- 인스턴스가 진행 중인 상태에서 오케스트레이터를 바꾸면 그 인스턴스의 재생이 깨집니다. 버전 1로 시작한 인스턴스는 버전 1의 히스토리에 대고 재생됩니다. 가운데에 액티비티 호출을 하나 끼워 넣고 배포하면 재생이 만나는 히스토리는 더 이상 코드와 맞지 않습니다. 런타임 자체의 답은 오케스트레이션 버저닝입니다. 모든 인스턴스에 시작 당시의 버전이 찍히고, 새 코드로 도는 오케스트레이터는 그 표시를 보고 분기해 옛 인스턴스를 계속 진행시키며, 옛 코드의 워커에는 새 인스턴스가 배정되지 않습니다. 새 task hub나 저장소 계정에 나란히 배포하는 것은 격리를 우선하는 대안이고, 진행 중인 인스턴스를 멈추는 방법은 프로토타입에나 맞습니다. 첫 릴리스 전에 하나를 골라 둡니다.
...
- isolated worker 모델에서는(2026년 11월에 in-process 지원이 끝나면 유일하게 지원되는 모델입니다) 오케스트레이터는 다른 것과 다를 바 없는 함수이고, 그것을 재생 가능하게 지켜 주는 API 표면은 `TaskOrchestrationContext` 하나입니다.
    ...
    [OrchestrationTrigger] TaskOrchestrationContext context)
{
    ...
    // 호출 하나하나가 체크포인트입니다. 결과가 히스토리에 기록되므로,
    // 재생은 액티비티를 다시 돌리는 대신 기록된 값을 돌려줍니다.
    var account = await context.CallActivityAsync<Account>(nameof(CreateAccount), signup);

    // 재시도 정책은 오케스트레이션이 아니라 액티비티 하나에 붙습니다.
    ...
    // 둘 중 어느 것도 스레드를 붙잡지 않습니다. 결정적인 시계를 눈여겨봅니다.
    // 기한은 DateTime.UtcNow가 아니라 context에서 나옵니다.
...
- 외부 이벤트는 클라이언트로 올리고, 인스턴스 id는 생성하지 말고 정해서 씁니다. 워크플로가 다루는 대상에서 끌어낸 id로 `ScheduleNewOrchestrationInstanceAsync`를 부르면 첫 인스턴스가 아직 실행 중이거나 대기 중인 동안에는 같은 주문에 대한 두 번째 인스턴스를 런타임이 거절해 주는데, 이보다 싼 중복 제거는 없습니다. 다만 그 인스턴스가 완료되거나 실패하거나 종료되고 나면 id는 다시 자유로워지므로, 주문당 최대 한 번을 지키려면 호출 앞에 상태 확인이 여전히 필요합니다. 승인 웹훅이 부르는 쪽은 이벤트 이름을 담은 `RaiseEventAsync`입니다.
- 저장소 공급자는 의식적으로 고릅니다. Azure Storage는 기본 내장 백엔드로 설정이 필요 없고 요금 모델이 가장 쌉니다. 관리형 Durable Task Scheduler는 권장 백엔드이고 처리량이 가장 높습니다. Microsoft SQL 공급자는 온프레미스와 연결이 끊긴 환경의 답입니다. 그리고 Netherite는 2028년 3월에 지원이 끝나는 퇴출 수순입니다. 이들 사이를 옮기는 일은 코드 변경이 아니라 재배포이고, 기존 상태를 옮겨 갈 마이그레이션 경로는 없습니다.
