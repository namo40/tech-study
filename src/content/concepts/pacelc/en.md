...
Written out, systems get a two-letter description. PC/EC pays for consistency in both branches, which is where Bigtable, HBase and a relational cluster whose reads go to the leader sit; PA/EL gives it up in both, which is Dynamo, Cassandra, Riak and a Cosmos DB account left at its default. The mixed pairs are the rare ones: PC/EL — consistent when the network breaks, fast the rest of the time — is the shape of a synchronous-commit availability group whose reads are served by readable secondaries, and Abadi's own survey struggles to name a store that lands there. The point of the notation is not to file products into boxes; it is to make you say the Else branch out loud. Two teams can agree completely on what happens during a partition and still be building different products, because one of them is paying a cross-region round trip on every read and the other is not.
