...
- Test the replay, not just the happy path. `WorkflowReplayer` runs a recorded history against the current code and fails if it no longer matches, which turns the versioning caution above into a check that runs in CI before a deployment breaks an instance that started last week.
