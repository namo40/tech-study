...
summary: "readiness probe は数秒ごとに 1 つのことを尋ねます。今このインスタンスがトラフィックを受けてよいか。失敗すればポッドは静かにローテーションから外れ、通過すればトラフィックが戻ります。liveness はもっと厳しい質問、つまりこのプロセスが存在し続けてよいかを尋ね、再起動で答えます。"
category: "コンテナーとオーケストレーション"
scene: readiness-probe
steps:
  - title: "2 つの質問、2 つの答え"
    text: "readiness は、今このインスタンスがトラフィックを受けてよいかを問います。liveness は、このプロセスが存在し続けてよいかを問います。起動したばかりのポッドは readiness に失敗して何も受け取らず、次に通ったプローブでローテーションへ加わります。"
  - title: "not ready は静かな退場です"
    text: "依存先が一瞬揺れると readiness が 3 回失敗し、ポッドは endpoints から外れます。何も殺されません。トラフィックがその横を流れるだけです。プローブが再び通れば、トラフィックは去ったときと同じくらい静かに戻ってきます。"
  - title: "liveness の答えは再起動です"
    text: "固まったプロセスは何も通せないので、readiness が追いつくまでトラフィックはまだ届いて失敗します。それから kubelet がコンテナーを殺して再起動します。カウンターが増え、readiness が新しい出発を守り、ポッドが準備できるまでトラフィックは待ちます。"
  - title: "liveness は自分を、readiness は依存先を見ます"
    text: "liveness にデータベースを確認させると、一度の揺れですべてのポッドが同時に再起動します。自分で作った障害です。配線を readiness へ移せば再起動は止まります。ポッドは戻り、データベースが落ちている間はローテーションの外で待ち、答えが返れば再び加わります。"
related:
  - label: Health Check
    slug: health-check
...
- 壊れてはいないのに一時的に処理できないインスタンスがある場所で使います。依存先を待っているポッド、負荷を捨てているポッド、停止を指示されて残りのリクエストを片づけているポッドは、いずれも殺されることではなく飛ばされることを望んでいます。
- ready の数をレプリカの数と並べて見て、その隣で再起動カウンターを見ます。ready の数が下がりながら再起動が増えていくなら、たいていは全体が病んでいるのではなく、プローブが見当違いのものを見ています。
...
- 負荷がかかると失敗する readiness の検査は、全体が揃っていてほしいまさにその瞬間に台数を減らします。検査がレイテンシやキュー長を測っていると、忙しいインスタンスが自分から外れ、その分が隣へ移り、隣も順に外れていきます。プローブを緩めるのではなく、負荷の切り捨てと同時実行数の制限を組み合わせます。
- 起動の遅いプロセスには、liveness を起動時間まで引き延ばすのではなく startup probe を使います。2 分の起動に耐えられるほど寛容な liveness probe は、固まったプロセスを 2 分放置するにも十分寛容です。
- 終了が始まったら readiness をすぐに失敗させます。endpoints からインスタンスを外す処理は瞬時ではないので、ready の報告をやめるのと同時にリクエストも断り始めるポッドは、ほんの少し前にルーティングされたリクエストを断ることになります。
- エンドポイントはクラスター内では認証なしで軽く保ち、公開ルーターには載せません。すべてのインスタンスで数秒ごとに、いつまでも走る検査なので、高価なものを問い合わせる検査は健全性という名前を付けた負荷生成器です。
...
readinessProbe:
  ...
  failureThreshold: 3
livenessProbe:
  httpGet: { path: /healthz/live, port: 8080 }   # プロセスだけを見ます
  periodSeconds: 10
  failureThreshold: 3
startupProbe:
  httpGet: { path: /healthz/live, port: 8080 }
  periodSeconds: 5
  failureThreshold: 30                           # 起動に最大 150 秒
```

最後の一片は終了です。`ApplicationStopping` が発生した瞬間に not-ready を報告し、ポッドを直接プローブするルーターが、処理中のリクエストが終わる間に仕事を送るのをやめるようにします。Kubernetes では、Service の endpoints は削除の時点ですでに知らされています。そしてホストの終了タイムアウトはプラットフォームの猶予期間の内側に収めます。

```csharp
// ライフタイムが立て、readiness が読むフラグ 1 つ。受け渡しはこれで全部です。
