...
summary: "readiness probe는 몇 초마다 한 가지를 묻습니다. 지금 이 인스턴스가 트래픽을 받아도 되는가. 실패하면 파드는 조용히 로테이션에서 빠지고, 통과하면 트래픽이 돌아옵니다. liveness는 더 가혹한 질문, 즉 이 프로세스가 계속 존재해도 되는지를 묻고 재시작으로 답합니다."
category: "컨테이너와 오케스트레이션"
scene: readiness-probe
steps:
  - title: "질문 두 개, 답 두 개"
    text: "readiness는 지금 이 인스턴스가 트래픽을 받아도 되는지 묻습니다. liveness는 이 프로세스가 계속 살아 있어도 되는지 묻습니다. 막 시작한 파드는 readiness에 실패해 아무것도 받지 않다가, 다음 프로브가 통과하는 시점에 로테이션에 합류합니다."
  - title: "not ready는 조용한 퇴장입니다"
    text: "의존 대상이 잠깐 흔들리면 readiness가 세 번 실패하고, 파드는 endpoints에서 빠집니다. 무엇도 죽지 않습니다. 트래픽이 그 옆으로 흘러갈 뿐입니다. 프로브가 다시 통과하면 트래픽은 떠날 때만큼 조용히 돌아옵니다."
  - title: "liveness의 답은 재시작입니다"
    text: "멈춰 버린 프로세스는 아무것도 통과시키지 못하므로, readiness가 따라잡을 때까지 트래픽은 그 파드에 떨어져 실패합니다. 그다음 kubelet이 컨테이너를 죽이고 다시 시작합니다. 카운터가 올라가고, readiness가 새 출발을 지키고, 파드가 준비될 때까지 트래픽은 기다립니다."
  - title: "liveness는 자신을, readiness는 의존성을 봅니다"
    text: "liveness가 데이터베이스를 확인하게 하면 한 번의 흔들림에 모든 파드가 동시에 재시작합니다. 스스로 만든 장애입니다. 연결선을 readiness로 옮기면 재시작이 멈춥니다. 파드는 돌아오고, 데이터베이스가 내려간 동안 로테이션에서 빠져 있다가, 응답이 돌아오면 다시 합류합니다."
related:
  - label: Health Check
    slug: health-check
...
- 인스턴스가 고장 나지 않은 채로 잠시 서비스할 수 없는 곳에 씁니다. 의존 대상을 기다리는 파드, 부하를 흘려보내는 파드, 멈추라는 말을 듣고 남은 요청을 비우는 파드는 모두 죽는 것이 아니라 건너뛰어지기를 원합니다.
- ready 개수를 레플리카 개수와 나란히 보고, 그 옆에서 재시작 카운터를 봅니다. ready 개수가 내려가는 동안 재시작이 올라간다면 대개 인스턴스 전체가 아픈 것이 아니라 프로브가 엉뚱한 곳을 보고 있다는 뜻입니다.

## 주의점

- readiness는 의존 대상을 확인해도 되지만, liveness는 프로세스 자신만 확인해야 합니다. 데이터베이스까지 손을 뻗는 liveness probe는 느린 쿼리 하나를 인스턴스 전체의 재시작으로 바꾸고, 그 재시작이 데이터베이스를 더 느리게 만듭니다.
...
- 부하가 걸릴 때 실패하는 readiness 검사는 인스턴스 전체가 온전해야 할 바로 그 순간에 그 수를 줄입니다. 검사가 지연 시간이나 큐 길이를 재면 바쁜 인스턴스가 스스로 빠지고, 그 몫이 이웃으로 옮겨 가고, 이웃도 차례로 빠집니다. 프로브를 느슨하게 하는 대신 부하 차단과 동시성 제한을 함께 둡니다.
...
- 종료가 시작되면 readiness를 곧바로 실패시킵니다. endpoints에서 인스턴스를 빼는 일은 즉시 끝나지 않으므로, ready 보고를 멈추는 순간에 요청도 함께 거절하기 시작하는 파드는 아주 조금 전에 라우팅된 요청을 거절하게 됩니다.
- 엔드포인트는 클러스터 안에서 인증 없이 가볍게 두고, 공개 라우터에는 올리지 않습니다. 모든 인스턴스에서 몇 초마다 영원히 도는 검사이므로, 비싼 것을 조회하는 검사는 건강이라는 이름을 단 부하 생성기입니다.
...
readinessProbe:
  ...
  failureThreshold: 3
livenessProbe:
  httpGet: { path: /healthz/live, port: 8080 }   # 프로세스만
  periodSeconds: 10
  failureThreshold: 3
startupProbe:
  httpGet: { path: /healthz/live, port: 8080 }
  periodSeconds: 5
  failureThreshold: 30                           # 기동에 최대 150초
```

마지막 조각은 종료입니다. `ApplicationStopping`이 발생하는 순간 not-ready를 보고해서, 파드를 직접 프로브하는 라우터가 남은 요청이 끝나는 동안 일을 보내지 않게 합니다. Kubernetes에서는 Service endpoints가 삭제 시점에 이미 통보를 받았습니다. 그리고 호스트의 종료 타임아웃은 플랫폼의 유예 시간 안에 둡니다.

```csharp
// 수명이 설정하고 readiness가 읽는 플래그 하나가 이 핸드셰이크의 전부입니다.
