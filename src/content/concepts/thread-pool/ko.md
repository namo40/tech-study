...
steps:
  ...
  - title: "막힘"
    text: "요청마다 I/O를 기다리는 동안 스레드를 쥐고 멈춥니다. 곧 모든 스레드가 기다리기만 하고, 큐는 쌓이고, 풀은 스레드를 한 번에 하나씩만 보탭니다. 이것이 Thread Pool starvation입니다."
  - title: "await"
    text: "같은 요청이 이번에는 I/O를 await합니다. 기다리는 동안 스레드를 내놓고, 끝나면 아무 빈 스레드로 돌아옵니다. 스레드 넷이 흐름 전체를 감당합니다."
  - title: "CPU 작업"
    text: "긴 계산은 정말로 스레드가 필요합니다. 동시에 도는 개수를 제한해 나머지 풀이 계속 요청을 처리하게 하고, 막히는 I/O를 Task.Run으로 감싸 비동기인 것처럼 보이게 하지 않습니다."
related:
...
- 고를 수 있는 것이 아닙니다. 모든 ASP.NET Core 요청, 모든 `Task` 연속 작업, 모든 타이머 콜백이 이미 풀 위에서 돕니다. 남는 질문은 풀을 굶기지 않는 방법뿐입니다.
...
`dotnet-counters monitor --counters System.Runtime`으로 `ThreadPool Thread Count`와 `ThreadPool Queue Length`를 실시간으로 볼 수 있습니다. 큐가 계속 자라는 동안 스레드 수가 초당 하나꼴로 올라간다면 그것이 starvation의 신호입니다.
