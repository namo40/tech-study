...
The rule that falls out of this is that the acknowledgement goes after the work and its side effects are durable, never before. Acknowledging on receipt, or on a timer, is the setting that quietly converts every crash, every deployment and every OOM kill into lost messages. `Azure.Messaging.ServiceBus` and `RabbitMQ.Client` make you ask for it; the one that is on by default is a Kafka consumer's `enable.auto.commit`, which moves the offset every five seconds whether the work behind it finished or not. A failure that the handler can see gets a negative acknowledgement instead, and there the choice is whether to requeue or to dead-letter: requeue for something that may work on the next attempt, dead-letter for something that will not. Brokers count deliveries so this decision can be made for you, and the count is worth reading in the handler as well, because the second attempt at a message is a good moment to log more and try less.
