...
여기서 나오는 규칙은 하나입니다. 확인 응답은 작업과 그 부수 효과가 저장된 뒤에 보내고, 그 전에는 보내지 않습니다. 받자마자, 또는 타이머에 맞춰 확인 응답을 보내는 설정은 모든 장애와 모든 배포와 모든 OOM 종료를 조용히 메시지 유실로 바꿔 놓습니다. `Azure.Messaging.ServiceBus`와 `RabbitMQ.Client`는 이 설정을 직접 요청해야 하고, 기본으로 켜져 있는 것은 Kafka 소비자의 `enable.auto.commit`입니다. 이것은 뒤의 작업이 끝났든 아니든 5초마다 offset을 옮깁니다. 핸들러가 알아챈 실패에는 부정 확인 응답을 보내고, 이때 큐로 되돌릴지 데드 레터 큐로 보낼지를 고릅니다. 다음 시도에서는 될 만한 것이면 되돌리고, 그럴 리 없는 것이면 데드 레터 큐로 보냅니다. 브로커가 전달 횟수를 세어 주므로 이 판단을 맡길 수 있고, 핸들러에서도 그 횟수를 읽어 볼 만합니다. 두 번째 시도는 로그를 더 남기고 작업은 덜 벌일 좋은 시점이기 때문입니다.
