...
풀은 두 종류의 스레드를 둡니다. Worker Thread는 큐에 들어온 것을 실행합니다. `Task.Run`의 본문, 타이머 콜백, 요청 핸들러의 동기 구간 같은 것들입니다. I/O 완료 스레드는 운영체제가 끝낸 작업을 이어받기 위해 있습니다. 덕분에 `await` 뒤의 continuation이 실행될 자리가 생기고, 그동안 워커 스레드가 놀면서 기다릴 필요가 없습니다. 둘 다 각자의 최소치와 최대치를 가지며, `ThreadPool.SetMinThreads`는 두 가지를 함께 설정합니다.
