...
summary: "memory pressure는 limit 근처에서 살아가는 힙입니다. 할당이 수집보다 빠르게 들어오면 수집기는 더 자주 돌고 더 오래 멈추며, limit을 넘는 순간에는 잡을 오류조차 없이 컨테이너가 그냥 종료됩니다."
category: "컨테이너와 오케스트레이션"
scene: memory-pressure
steps:
  - title: "limit에서 먼 힙은 GC를 거의 느끼지 못합니다"
    text: "할당이 채우고 수집이 비우며 게이지는 30%와 50% 사이에서 숨을 쉽니다. 수집은 값싼 배경 소음입니다. 이것이 건강한 모습이고, 아무도 이 그래프를 들여다보지 않습니다."
  - title: "압박은 빈도로 먼저 나타납니다"
    text: "할당 속도가 3배가 되면 수집기는 몇 배나 자주 돌고(판독값이 4/min에서 20을 넘어 올라갑니다), 매번 더 오래 멈추고, 한 번에 덜 비워 냅니다. 힙은 이제 천장 근처에서 살고, 모든 할당이 그 대가를 치릅니다."
  - title: "limit은 아무것도 던지지 않습니다"
  ...
  - title: "답은 더 큰 limit이 아니라 더 적은 할당입니다"
    text: "큰 버퍼를 풀로 만들어 빌리고, 쓰고, 반납하면 속도가 무너져 내립니다. 같은 트래픽을 다시 흘려도 게이지는 절반 언저리로 내려앉고 GC는 조용해집니다. 더 큰 limit은 절벽을 옮겨 놓을 뿐입니다. 재시작 횟수는 그대로 남습니다. 흉터가 곧 교훈입니다."
related:
...
- 경보가 아니라 판독으로 씁니다. 세 숫자는 함께 놓아야 뜻이 생깁니다. 수집기가 얼마나 자주 도는지, 한 번에 얼마나 오래 멈추는지, 힙이 limit에 얼마나 가까운지입니다. 하나씩 떼어 놓으면 아무 의미가 없습니다. 여유 있는 힙에서 분당 20번 수집은 정상이고, 아무것도 채우지 않는다면 80%에 머무는 힙도 정상입니다. 압박은 이 셋의 조합이며, 03시 14분에 일어난 사건이 아니라 서비스가 놓여 있는 상태입니다.
...
- 메모리 limit을 올리기 전에 봅니다. limit은 바꾸기가 가장 쉬우면서 바꿔야 할 이유는 거의 없는 숫자입니다. 먼저 상태를 읽습니다. 512Mi에서 죽는 컨테이너와 2Gi에서 죽는 컨테이너는 같은 결함을 가진 같은 컨테이너이고, 뒤쪽은 거기까지 가는 데 더 오래 걸리고 도착했을 때 더 오래 멈출 뿐입니다.

## 주의점

- 플랫폼이 강제하는 limit과 런타임이 믿고 있는 limit은 같은 숫자여야 합니다. .NET은 cgroup에서 컨테이너 limit을 읽고 그 일부를 힙 크기로 잡으므로 기본값은 대개 맞습니다. 다만 누군가 `DOTNET_GCHeapHardLimit`을 손으로 설정했거나, cgroup v2를 지원하지 않는 오래된 런타임이거나, 런타임이 볼 수 없는 방식으로 파드에 limit이 걸린 순간부터는 맞지 않게 됩니다. 런타임이 커널이 허락한 것보다 여유가 많다고 믿으면 수집기는 끝내 충분히 공격적으로 돌지 않고, GC가 아직 느긋한 동안 종료가 찾아옵니다.
...
- 더 큰 limit은 시간을 벌어 주고 그 대가를 청구합니다. limit을 두 배로 늘리면 절벽의 위치만 옮겨 갈 뿐 기울기는 그대로이고, 멈추는 시간은 수집기가 훑어야 하는 라이브 세트에 대략 비례합니다. 40분마다 죽던 서비스가 이제 80분마다 죽고, 가는 길에 매번 두 배씩 멈춥니다. 진짜 수정을 준비하는 동안 서비스를 살려 두려고 일부러 그렇게 하는 것은 괜찮습니다. 그것이 수정인 적은 없습니다.
- 할당 속도가 지렛대이고, 사실상 유일한 지렛대입니다. 큰 버퍼를 풀로 돌리고, 응답을 버퍼에 담는 대신 흘려보내고, 복사 대신 `Span<T>`으로 잘라 쓰고, 핫 패스에서 아예 할당하지 않는 변경이 상태를 바꿉니다. 수집기 튜닝은 대개 바꾸지 못합니다. Server GC는 짧은 정지를 얻는 대신 메모리 사용량을 더 쓰고 `GCConserveMemory`는 작은 힙을 얻는 대신 처리량을 내주지만, 둘 중 어느 것도 프로세스가 쓰레기를 만들어 내는 일을 멈추지는 못합니다.
...
- 메모리는 CPU가 아니고, 두 limit은 같은 방식으로 고장 나지 않습니다. CPU limit을 넘으면 스로틀링되어 느려지고, 메모리 limit을 넘으면 종료되어 사라집니다. 이 비대칭 때문에 메모리 limit에는 CPU limit이 필요로 하지 않는 여유가 필요합니다. 그리고 메모리 limit을 request와 같게 두는 것은(파드 안의 모든 컨테이너에서 CPU limit과 request도 같게 둔다면 파드를 Guaranteed로 만드는 설정입니다) 천장에 대한 결정이면서 동시에 축출 우선순위에 대한 결정이기도 합니다.

## .NET에서는

런타임은 기본적으로 컨테이너를 인식합니다. cgroup의 메모리 limit을 읽어 그 75%를 힙 하드 리밋으로 잡고, 나머지는 스택과 JIT와 네이티브 할당처럼 관리 힙이 아닌 나머지 전부에 남겨 둡니다. 그래서 실제로 따져야 할 숫자는 컨테이너의 limit이고, 아래 설정은 그 기본 판독이 틀린 경우를 위한 것입니다.

```jsonc
// runtimeconfig.json. 이유를 모르면 아무것도 설정하지 않습니다. 기본값은
// 컨테이너 limit에서 파생되고, 이것을 덮어쓰는 것이 프로세스가 커널이 주지
// 않을 여유를 가졌다고 믿게 되는 경로입니다.
{
  "configProperties": {
    // 바이트 단위의 명시적 상한, 또는 컨테이너 limit에 대한 비율.
    "System.GC.HeapHardLimit": 402653184,
    // HeapHardLimit이 설정되어 있는 동안은 무시됩니다. 둘은 짝이 아니라 양자택일입니다.
    "System.GC.HeapHardLimitPercent": 75,
    // Server GC는 코어당 최대 힙 하나를 두고, DATAS(.NET 9부터 기본으로 켜짐)는
    // 하나에서 시작해 그 안으로 늘어납니다. 정지는 짧아지고 사용량은 커지는데,
    // limit에 그만한 여유가 있어야 감당할 수 있는 거래입니다.
...
플랫폼 쪽에서 limit은 한 줄이고, 그 limit에 닿았을 때의 진단도 한 줄입니다. OOM으로 종료된 파드는 마지막 상태에 그렇게 적어 두며, 뒤에 스택 트레이스가 없는 종료 코드가 함께 남습니다.
...
재시작 횟수가 올라가고 있는데 진짜 수정이 일주일 뒤라면 limit을 올리되, 그것이 집행유예일 뿐이라고 소리 내어 말해 둡니다. 그다음에 요청마다 프로세스가 무엇을 할당하는지 찾아봅니다. 그 숫자가 상태를 이루는 값이고, 그래프의 크기가 아니라 모양을 바꾸는 유일한 값입니다.
