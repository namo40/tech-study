...
- 엔티티 하나가 여러 항목에 흩어져 있을 때 씁니다. 상품 하나가 상세 페이로드로도, 목록 페이지 세 곳의 행으로도, 가격 조각으로도, 검색 결과로도 캐시되어 있고, 그 상품을 고치면 다섯 개가 전부 물러나야 합니다. 키로 지워 없애려면 쓰기 시점에 다섯 개의 키를 전부 알아야 하는데 쓰기 경로는 그것을 거의 알지 못합니다. 다섯 개 모두에 `product:42`를 붙여 두면, 그 지식이 항목의 모양이 아직 눈앞에 있는 채움 시점의 라벨로 바뀝니다.
...
- 새 키 공간을 만들지 않고 cache-version의 효과만 얻고 싶을 때 고릅니다. 버전 방식은 세대 번호를 키에 넣고 옛 항목이 만료로 사라지기를 기다려 같은 문제를 풉니다. 비용이 싸지만 쓰레기가 한동안 남습니다. HybridCache에서 태그 제거는 삭제가 아니라 논리적 차단선입니다. 읽는 쪽은 더 이상 옛 항목에 닿지 못하지만 바이트는 만료될 때까지 남아 있으므로, 메모리를 비우는 속도는 버전 방식보다 빠르지 않습니다. 태그는 공간이 아니라 도달 가능성을 위해 고릅니다.
...
- 태그가 넓을수록 구덩이가 큽니다. 카탈로그 항목 전부에 붙인 `catalog` 같은 태그는 무효화를 손쉽게 옳도록 만들어 주는 대신, 가격 한 줄 수정을 영역 전체의 찬 캐시로 바꿉니다. 그리고 그 결과는 하필 누군가 지켜보고 있는 순간에 데이터베이스로 몰려드는 스탬피드로 도착합니다. 쓰기 한 번이 정당하게 부술 수 있는 범위에 맞춰 태그 크기를 정하고, 넓은 태그 옆에 좁은 `product:42`를 함께 두어 흔한 경우가 작은 망치를 쓰게 합니다.
- 태그의 비용은 저장소에 따라 다릅니다. HybridCache는 태그마다 타임스탬프 하나를 두고 읽을 때 대가를 치릅니다. 찾아낸 항목을 그 태그의 마지막 무효화 시각과 비교하므로 비용은 읽기당 태그 수에 비례합니다. Redis 출력 캐시 같은 태그 집합 저장소는 대신 태그에서 항목으로 가는 역방향 대응을 들고 있는데, 이는 채움마다 태그당 쓰기 하나가 늘고 축출마다 일이 늘어난다는 뜻입니다. 공유 저장소에서는 그 색인도 공유되므로 수다스러운 태그 설계는 그 인스턴스를 쓰는 모두에게 지연으로 나타납니다. 어느 쪽이든 항목당 태그 몇 개까지는 감당할 만하고, 태그 달기가 자유 서술이 되는 순간부터 감당할 수 없어집니다.
...
```csharp
// 태그는 항목과 함께, 그 모양이 정해지는 순간에 선언합니다.
...
// 쓰기 경로는 키가 아니라 주제를 부릅니다.
await cache.RemoveByTagAsync($"product:{id}", ct);

// 넓은 태그는 섹션 전체를 다시 낼 때를 위해 있습니다. 의도적으로 씁니다.
await cache.RemoveByTagAsync("catalog", ct);
```

- 출력 캐시에도 응답을 위한 같은 장치가 있습니다. 엔드포인트에 붙인 `[OutputCache(Tags = ["catalog"])]`가 거기서 만들어진 캐시 변형 전부에 라벨을 달고, 관리 동작이나 메시지 핸들러에서 부르는 `IOutputCacheStore.EvictByTagAsync("catalog", ct)`가 그 전부를 물러나게 합니다. 축출하는 코드는 경로도 쿼리 문자열도 알 필요가 없습니다.
- 태그는 키를 만드는 그 자리에서 함께 만듭니다. 도우미가 `product:{id}`를 키로 만든다면 태그도 같은 도우미가 만들게 합니다. 그래야 규칙 이름이 바뀌었을 때 두 철자가 어긋난 채 모든 제거가 조용히 아무 일도 하지 않는 호출이 되어 버리는 일을 막습니다.
