...
steps:
  ...
  - title: "こちらのページだけが返せるトークン"
    text: "サーバーはフォームの中に antiforgery トークンを隠しておきます。本物の送信は Cookie とトークンを一緒に返し、トークンを持たない古いフォームはそれだけで 400 になり、偽造された POST はそのどちらも持って来られません。SameSite とオリジン確認を重ねて、どの一線も単独で持ちこたえなくて済むようにします。"
related:
