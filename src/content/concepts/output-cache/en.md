...
- It is not HTTP caching, and confusing the two produces surprises in both directions. Output caching is controlled by the server and ignores the `Cache-Control` a client sends, though it does still answer that client's conditional request with a 304, while response caching middleware is the header-driven one that honours `Cache-Control` and can be defeated by any client that sends `no-cache`. If the goal is to keep a CDN and a browser from re-fetching, that is a headers conversation; if the goal is to stop your own server from doing the work twice, this is the one.

## In .NET

- Output caching is built into ASP.NET Core: `AddOutputCache` registers it, `UseOutputCache` puts it in the pipeline after `UseCors`, `UseRouting`, `UseAuthentication` and `UseAuthorization` and before the endpoints, because it has to know whether a request is authenticated in order to skip it, and an endpoint opts in with `CacheOutput()` or the `[OutputCache]` attribute. Nothing is cached until something opts in.
...
- Expiration here is a plain TTL. There is no background refresh, but two of the things people assume are missing are on by default: concurrent misses for the same key are serialized, which `SetLocking(false)` turns off, and a conditional request is answered with a 304 from the cached entry with no configuration at all. What you do not get is stale-while-revalidate, so an expired entry is a miss for the one request that refills it; keep the lifetime long enough that the refill is rare, and reach for HybridCache at the data layer when the expensive part deserves collapsing.
