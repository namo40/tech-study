...
summary: "パラメーター化されたクエリは、文と値を別々の 2 つとして送るので、値はテキストがすでにパースされたあとに束縛されます。境界は気をつけるべき文字の一覧ではなく、構造です。"
...
シーンの 2 番目のステップを見ながら、実際に変わったものがどれだけ少ないかを確かめてみてください。同じ入力で、色も赤のままで、同じデータベースに届きます。ろ過したものも削り取ったものもありません。変わったのは App から出て行ったものの形です。1 番目のステップでは入力が文の末尾に溶接され、全体が 1 つのかたまりとして移動していました。2 番目では、値があった場所にプレースホルダーがあり、値は文の横を自分だけの板として移動します。2 つのあいだの隙間を隙間として描いているのは、その隙間こそが仕組みだからです。

この隙間が買ってくるのは順序です。サーバーは文のテキストを受け取り、木にパースして意味を確定し、そのあとで、木がすでに持っている穴に値を束縛します。パーサーにとって入力が存在し始めるころには、パースはもう終わっています。値の中の引用符が文字列リテラルを終わらせることはできません。その値は文字列リテラルの中にはなく、文法がすでに閉じた場所に置かれたパラメーターだからです。2 番目のステップの同じ攻撃文字列が危険でもなく、消毒されたわけでもない理由がこれです。それはただ、誰もそう呼ばれていない名前にすぎません。
...
隙間を越えていっしょに渡るもう 1 つは型です。パラメーターとして送った日付は日付であって、カルチャに左右されない書式に直してからまたパースし直す文字列ではありません。`decimal` は小数桁を保ち、null はそれを綴った 4 文字ではなく `DBNull.Value` です。どれもそもそも起きないバグの一群なのですが、起きなかったバグは痕跡を残さないので見落としやすいところです。

実務では、これらすべてが結局は習慣 1 つに落ち着きます。`FromSql` と `FromSqlRaw` は同じに見える文字列を受け取って正反対に扱いますし、`+` で作った `SqlCommand` はパラメーターで作ったものとほとんど同じ見た目です。安全なほうを指が先に打つ形にして、コードレビューには diff に出てくる SQL ごとに問いを 1 つだけ投げさせてください。値はどこから入ったのか。
