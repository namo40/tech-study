...
steps:
  - title: "로그인은 문을 열 뿐, 모든 서랍을 열지는 않습니다"
    text: "인가 단계가 없으면 로그인한 사용자는 무엇이든 만질 수 있습니다. 고스트는 B가 A의 문서에 쓰는 데 성공하는 모습을 보여 줍니다. 인증은 \"누구인가\"에 답하고, 인가는 \"해도 되는가\"에 답합니다. 안전은 두 번째 질문에 삽니다."
  - title: "역할은 이름이 붙은 권한 꾸러미입니다"
    text: "게이트는 배지를 봅니다. editor는 쓸 수 있고 viewer는 읽을 수 있고, viewer의 쓰기는 문서에 닿기 전에 거절됩니다. 역할이 있어야 사용자 천 명이 감당됩니다. 사람마다 목록을 주는 대신 꾸러미를 주기 때문입니다."
...
- 서비스끼리 부르는 경우에도 필요합니다. 유효한 토큰은 어느 서비스가 부르는지를 증명할 뿐, 그 서비스가 무엇을 해도 되는지는 말하지 않습니다. 머신 신원으로 도는 백그라운드 작업자도 사람과 똑같이 권한을 따로 정해야 하고, 대개 사람보다 적게 줘야 합니다.
...
- 로그인 성공은 권한이 아닙니다. 이것을 권한으로 취급하면 접근 제어가 무너지고, 이 결함은 실제 웹 취약점 목록에서 늘 상위에 있습니다. 장면의 1단계가 바로 그 모습입니다. 호출자가 로그인했다는 이유로 요청이 통과하고, 이 호출자가 이 문서를 만져도 되는지는 아무도 묻지 않습니다.
...
- 막힌 쪽으로 실패하게 만듭니다. 매핑되지 않은 경로, 아무도 특성을 붙이지 않은 컨트롤러, 판단하지 않고 그냥 돌아오는 인가 핸들러는 전부 거부로 끝나야 합니다. 기본 거부가 있어야 시스템이 허용하는 것의 목록과 적어 둔 것의 목록이 같아집니다.
...
- 판단은 한 곳에서 한 번만 내립니다. 컨트롤러 여기저기에 흩어진 인가 로직은 시간이 지나면 어긋나고, 갱신을 잊은 사본이 구멍이 됩니다. 규칙에 정책 이름을 붙여 두고 그 이름만 쓰면, 규칙 자체를 읽고 테스트하고 고칠 수 있게 됩니다.
...
역할은 굵직한 쪽이고, 다른 것과 마찬가지로 하나의 클레임입니다.

```csharp
// 이름 붙은 권한 꾸러미입니다. 여기서는 문서에 대해 아무것도 모릅니다.
...
답이 리소스에 달리는 순간부터는 역할로 표현할 수 없고 정책이 필요합니다. 요구 사항은 질문을 가리키는 표지이고, 핸들러가 그 질문에 답합니다. 리소스를 받는 쪽은 제네릭 인자가 둘인 `AuthorizationHandler`입니다.
...
public sealed class OwnerHandler : AuthorizationHandler<OwnerRequirement, Document>
{
        ...
        AuthorizationHandlerContext context, OwnerRequirement requirement, Document document)
    {
        ...
        // 명시적으로 성공을 알리고, 아니면 아무 말도 하지 않습니다. Succeed를 부르지
        // 않은 핸들러는 아무것도 허용하지 않은 것이고, 그것이 기본 거부입니다.
...
무언가 리소스를 건네주기 전에는 핸들러가 돌 수 없으므로, 엔드포인트는 문서를 먼저 읽고 나서 묻습니다. 이 순서가 핵심입니다. 판단은 그 행을 놓고, 호출이 일어나는 시점에 내려집니다.
...
작은 습관 두 가지도 값어치를 합니다. 정책 안에서는 역할 이름보다 권한 모양의 클레임을 씁니다. `RequireClaim("permission", "documents.write")`는 조직 개편으로 역할 이름이 전부 바뀌어도 살아남고, 꾸러미 정의를 여러 특성에 흩어 놓지 않고 한곳에 모아 둡니다. 그리고 인증은 됐지만 권한이 없는 호출자에게는 `404` 대신 `403`을 돌려줍니다. 리소스의 존재 자체가 비밀이라고 판단한 경우만 예외입니다. `Results.Forbid()`와 `Results.Challenge()`는 뜻이 다르고, 이 둘을 섞으면 로그인한 사용자가 로그인 화면을 계속 맴돌게 됩니다. `Forbid`는 스킴에 위임하므로 호출자가 실제로 받는 것은 어느 스킴이 답했느냐에 달려 있습니다. JWT bearer 핸들러는 `403`을 쓰고, 쿠키 핸들러는 `OnRedirectToAccessDenied`를 재정의하지 않는 한 `AccessDeniedPath`로 리다이렉트합니다.
