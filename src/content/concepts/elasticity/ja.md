...
scene: elasticity
steps:
  - title: "固定容量は一日に二度失敗します"
    text: "朝のピークにはポッド三つが溺れ、ユーザーが待ちます。深夜 3 時には同じ三つが遊んでいるのに、料金は走り続けます。ピークに合わせれば夜が無駄になり、夜に合わせれば朝が壊れます。弾力性はその選択を拒みます。容量が需要を推測する代わりに、追いかけます。"
  - title: "まず水平です。同じポッドを増やします"
    text: "需要が上がり平均 CPU が目標を超えると、オートスケーラーがレプリカを足します。一つのサービスの後ろに同じポッドが並び、それぞれが分け前を受け持ちます。需要が下がれば余分は抜けていきますが、遅れを置いて抜けます。性急な縮小はあらゆるさざ波を再起動の嵐に変えるからです。"
  - title: "ポッドそのものが間違ったサイズなら垂直です"
    text: "一年前に書いたリクエスト値が今日の負荷と出会います。あるポッドは小さすぎる箱の中で飢え、別のポッドは触りもしないメモリを抱え込みます。垂直オートスケーラーは再起動という代価を払ってリクエスト値を正します。そして正直なリクエスト値は、他のすべての層の計算が寄りかかる土台です。"
...
- 時間帯やイベントで上下する需要です。一日の周期、キャンペーン、バッチ枠のように、トラフィックのグラフが水平線ではなく形を持っている場合です。
- 「念のため」に残した遊休容量に料金を払っていて、その「念のため」が一日の大半を占めている場合です。
- 固定した規模では吸収できないピークで遅延の予算が壊れる場合です。ピークに合わせて買うと、週に数時間のために買うことになります。
- レプリカ数やリソースのリクエスト値を最後に見直してから何か月も経っている Kubernetes のワークロードです。その数字は、すでに変わってしまったワークロードに合っていた値です。

## 注意点

- 弾力性には動いてくれる信号が要ります。アプリケーションが実際に詰まる場所ではない指標でオートスケールすると、見当違いのものが高くつく形で増えます。データベースを待っているサービスについて CPU は何も語りませんし、すでに飽和したコンシューマーグループのキューにレプリカを足しても接続が増えるだけです。
...
- 危ないのは縮小の方向です。レプリカを減らすことは、そのレプリカが抱えていたものを退去させることなので、中断予算と丁寧な終了と安定化ウィンドウを守る必要があります。増やすときは積極的に、減らすときは辛抱強くというのは一貫性のなさではなく、それ自体が要点です。
...
仕事の大半はマニフェストではなくアプリケーション側にあります。水平に増えるサービスは、二つ目のコピーが必要とするものをローカルに抱えず、止まれと言われたら手元の仕事を仕上げ、実際に応答できるようになるまでは自分を準備できていないと報告する必要があります。

```csharp
var builder = WebApplication.CreateBuilder(args);

// セッション状態はポッドの外に置きます。どのレプリカでもどの要求に答えられます。
...
// 縮小は退去です。プロセスが去る前に手にした仕事を終わらせます。
