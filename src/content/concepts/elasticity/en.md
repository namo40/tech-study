...
steps:
  ...
  - title: "Horizontal first: more of the same pod"
    text: "Demand climbs, average CPU crosses the target, and the autoscaler adds replicas — identical pods behind one service, each taking a share. Demand falls and the extras drain away, on a delay, because scaling in too eagerly turns every ripple into a restart storm."
  - title: "Vertical when the pod itself is the wrong size"
    text: "Requests written a year ago meet today's workload: one pod starves inside a too-small box while another hoards memory it never touches. The vertical autoscaler re-rights the requests — at the price of a restart — and honest requests are what every other layer's math depends on."
...
- Scale-in is the dangerous direction. Removing a replica evicts whatever it was holding, so it has to respect disruption budgets, graceful shutdown and a stabilisation window. Scaling out eagerly and in patiently is not an inconsistency; it is the whole asymmetry.
...
// Session state lives outside the pod, so any replica can answer any request.
