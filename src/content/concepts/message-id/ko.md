...
브로커가 이 일의 일부를 대신해 주기도 합니다. Azure Service Bus는 `MessageId`를 받고, 엔티티에 중복 감지를 켜 두면 설정한 기간 안에 이미 본 id를 가진 두 번째 메시지를 버립니다. 이 기간은 기본 10분이고 20초에서 7일 사이로 잡을 수 있습니다. 불안정한 발행자가 만들어 내는 반복은 아무에게도 닿기 전에 사라지는데, 보내려고 했던 것 일부도 함께 사라집니다. 예약 메시지도 검사 대상이라서, 원래 id를 그대로 가진 재시도 사본이나 데드 레터 재투입은 기간이 열려 있는 동안 보냈다고 보고된 뒤 버려집니다. 다만 소비자가 메시지를 처리한 뒤 확인 응답을 보내기 전에 죽어서 생기는 반복은 그대로 남으므로, id는 우리 코드에도 보여야 합니다. correlation id와도 구분해 둡니다. 메시지 id는 이 메시지의 이름이고, correlation id는 이 메시지가 속한 대화의 이름입니다. 한쪽을 다른 쪽으로 쓰면 한 흐름의 모든 메시지가 첫 메시지의 중복처럼 보이게 됩니다.
