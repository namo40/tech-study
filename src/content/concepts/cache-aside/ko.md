---
title: "Cache-Aside"
summary: "Cache-Aside는 캐시를 애플리케이션이 직접 관리하는 방식입니다. 먼저 캐시를 읽고, miss면 데이터베이스로 내려가 그 결과를 캐시에 저장하며, 데이터가 바뀌면 엔트리를 무효화합니다."
category: "캐시"
scene: cache-aside
steps:
  - title: "Miss"
  ...
  - title: "TTL"
    text: "엔트리는 만료됩니다. 그다음 읽기가 miss가 되어 캐시를 다시 채우므로, 오래된 값이 TTL보다 오래 살아남을 수 없습니다."
  - title: "쓰기"
    text: "데이터베이스만 갱신하는 쓰기는 캐시를 오래된 상태로 남깁니다. 쓸 때 엔트리를 무효화하면 그다음 읽기가 새 값으로 다시 채웁니다."
related:
...
- 무효화를 하더라도 모든 엔트리에 TTL을 둡니다. 놓친 무효화를 받아 주는 안전망입니다.
- Cache-Aside 자체는 stampede를 막지 않습니다. 인기 있는 키가 만료되면 많은 읽기가 한꺼번에 miss가 되므로, 요청 병합이나 Stale-While-Revalidate와 함께 씁니다.
...
`HybridCache`는 같은 키에 동시에 발생한 miss를 호출 하나로 합쳐 줍니다. 직접 만들어야 했을 stampede 보호를 대신 해 주는 셈입니다. `IDistributedCache`가 등록되어 있으면 그것이 2차 캐시가 되어, Redis 같은 공유 캐시가 프로세스 안의 캐시 뒤에 자리 잡습니다.
