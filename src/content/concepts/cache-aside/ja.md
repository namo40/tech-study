---
title: "Cache-Aside"
summary: "Cache-Aside はキャッシュをアプリケーション自身が管理する方式です。まずキャッシュを読み、ミスならデータベースまで下りてその結果をキャッシュに保存し、データが変わったらエントリを無効化します。"
category: "キャッシュ"
scene: cache-aside
steps:
  - title: "ミス"
  ...
  - title: "TTL"
    text: "エントリは期限切れになります。次の読み取りがミスになってキャッシュを埋め直すので、古い値が TTL より長く生き残ることはありません。"
  - title: "書き込み"
...
- Cache-Aside 自体はスタンピードを防ぎません。人気のあるキーが期限切れになると多くの読み取りが一斉にミスするので、リクエストの合流や Stale-While-Revalidate と組み合わせます。
...
`HybridCache` は同じキーに同時に起きたミスを 1 回の呼び出しにまとめてくれます。自分で書くことになっていたスタンピード対策を代わりに担ってくれる形です。`IDistributedCache` が登録されていればそれが 2 次キャッシュになり、Redis のような共有キャッシュがプロセス内のキャッシュの後ろに置かれます。

上の形が隠している細部が 1 つあります。`FindAsync` が返した `null` も、ほかの値と同じように同じ 5 分の期限でキャッシュされます。エントリのオプションはファクトリーが走る前に固定されるからです。存在しないキーがまとめて届くなら、これは 4 番目の注意点が求める短時間のネガティブキャッシュではありません。番兵の値と短い TTL を組み合わせたその形は Negative Cache のページにあります。
