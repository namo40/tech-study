...
steps:
  - title: "저장소는 둘, 약속은 하나"
    text: "주문과 결제는 둘 다 일어나거나 둘 다 일어나지 않아야 합니다. 따로 커밋하면 한쪽은 예라고 했는데 다른 쪽은 아니라고 한 순간이 생기고, 고객에게 남는 것은 그 반쪽 상태입니다. 누군가는 두 데이터베이스를 하나처럼 움직이게 만들어야 합니다."
  ...
  - title: "약속의 대가는 기다림입니다"
    text: "prepared는 잠겼다는 뜻입니다. 그 순간 코디네이터가 죽으면 두 저장소는 잠금을 쥔 채 기다리고, 그 행이 필요한 모든 것이 함께 기다립니다. 이 미결 구간이야말로 현대 시스템이 사가와 아웃박스를 먼저 찾는 이유입니다."
related:
...
- 대안이 아무도 보상하지 않는 반쪽 세계일 때입니다. 서비스를 넘나드는 원자성에는 사가가 대체로 맞는 답이지만, 그건 누군가 보상 로직을 실제로 작성하고 테스트했을 때의 이야기입니다. 내가 소유한 데이터베이스 두 개에 2단계 커밋을 쓰는 쪽이, 다이어그램으로만 존재하는 사가보다 정당한 선택일 수 있습니다.
...
{
    await orders.OpenAsync(ct);        // 여기까지는 아직 로컬 트랜잭션이다
...
```csharp
// Windows에서만, MSDTC로만 된다. Linux나 대부분의 컨테이너에서는 설정으로
// 풀 수 있는 문제가 아니다. 트랜잭션 관리자가 아예 없기 때문이다.
...
두 번째 문장이 사람들이 2단계 커밋에 바라는 것 대부분에 대한 현대적인 답입니다. 무슨 일이 있었는지 다른 서비스에 알리는 메시지를 변경 자체와 같은 로컬 트랜잭션 안에서 기록하므로, 둘이 어긋날 수가 없습니다. 별도 프로세스가 아웃박스를 읽어 나중에 발행합니다. 네트워크 호출을 사이에 두고 붙들고 있는 것이 없고, 반대편은 원자적이 아니라 최종 일관성을 갖게 됩니다. 이것이 정직한 거래이며, 의식적으로 선택할 만한 거래입니다.

```csharp
// 반대편은 나머지 절반을 맡는다. 두 번 실행돼도 해가 없는 단계여야 한다.
// 아웃박스가 주는 것은 at-least-once 배달이기 때문이다. 아래 검사는 최적화일 뿐
// 방어선이 아니다(청구와 원자적이지 않다). 그래서 `ChargeAsync`가 쓰는 행은
// `Charges.OrderId`의 unique index 뒤에 있고, 주문 id를 결제 제공자의
// idempotency key로 보낸다.
...
작업이 정말로 여러 서비스에 걸쳐 있고 정말로 전부 아니면 전무여야 한다면, 그 모양은 사가입니다. 각 단계는 로컬로 커밋하고, 나중 단계가 실패하면 보상을 따라 거꾸로 걸어 나옵니다. 어떤 관찰자도 반쯤 바뀐 세계를 보지 않는다는 보장을 포기하는 대신, 2단계 커밋이 잠금에 쓰는 비용을 전부 되찾습니다. 그 창을 감수하겠다고 판단해서 고릅니다. 프로토콜을 설정하기가 번거로워서 고르는 것이 아닙니다.
