...
steps:
  - title: "Committed or nothing"
    text: "Two transactions share one row. While the first is mid-change, the second reads, and sees only what was last committed, never the half-done value. That floor is read committed, and it is where almost every database starts you."
  ...
  - title: "Versions instead of locks"
    text: "Snapshot reads the world as it was when you began, so nobody waits. Writes still collide: when two transactions change the same row, the version number betrays the loser, one rolls back and retries. Optimism moves the cost from waiting to retrying."
...
- Learn your database's default before you tune anything. SQL Server ships read committed with shared locks, PostgreSQL ships read committed with row versions, and turning on `READ_COMMITTED_SNAPSHOT` in SQL Server changes what read committed means without changing a line of your code.
...
- Phantom rows survive further than most people expect. Repeatable read protects the rows you already touched, not the rows that arrive afterwards, so a `COUNT` inside a repeatable-read transaction can still grow. Closing that needs serializable or an explicit range lock.
- The level does not travel. It applies to the connection for the life of the transaction, so code that opens a second connection inside the same logical operation is running at the default there, and code that returns a lazy sequence has usually closed the transaction before the caller finishes reading.
