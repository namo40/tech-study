...
그래서 코드의 올바른 모양은 뛰기 전에 살펴보는 것이 아닙니다. 일단 삽입하고, 실패를 답으로 받는 것입니다. `SaveChangesAsync`는 `DbUpdateException`을 던지고, 그 아래에는 "고유 위반"을 콕 집어 말하는 공급자 오류 번호가 있습니다. SQL Server라면 2601이나 2627, PostgreSQL이라면 `23505`입니다. 그것이 `409 Conflict`를 돌려주거나, 이미 있는 행을 읽어 그대로 이어 가라는 신호입니다. 흔한 실수는 모든 `DbUpdateException`을 똑같이 잡는 것입니다. 외래 키 위반과 타임아웃도 같은 외투를 입고 오는데, 그것들까지 "이미 있음"으로 삼켜 버리면 진짜 버그가 가려집니다. 바깥 예외가 아니라 안쪽 예외를 보고 가릅니다.

선언하기 전에 알아 둘 것이 셋 있습니다. `NULL`은 자기 자신과 같지 않아서 PostgreSQL, MySQL, SQLite는 고유 컬럼에 `NULL`을 담은 행을 여럿 허용하지만, SQL Server는 `NULL` 둘을 같다고 보고 정확히 하나만 허용합니다. SQL Server에서 반대 동작을 얻는 방법은 필터 고유 인덱스(`HasFilter("[Email] IS NOT NULL")`)이고, 어느 쪽도 원하는 바가 아니라면 그 컬럼은 널을 허용하지 않아야 합니다. 여러 컬럼을 묶은 고유성은 각각의 고유성과 다른 규칙입니다. `HasIndex(x => new { x.TenantId, x.Email }).IsUnique()`는 테넌트마다 이메일 하나라는 뜻이고, 전역 이메일에 대해서는 아무 말도 하지 않습니다. 그리고 비교 규칙은 우리 것이 아니라 컬럼의 것입니다. `Ann@example.com`이 `ann@example.com`과 부딪히는지는 콜레이션이 정하므로, 바라지 말고 들어올 때 값을 정규화합니다. 행을 소프트 삭제한다면 필터(`HasFilter("[DeletedAt] IS NULL")`)를 붙여, 삭제된 행이 그 값을 영원히 잡아 두지 않게 합니다.
