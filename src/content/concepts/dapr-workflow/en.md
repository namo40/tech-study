...
- The sidecar is a runtime you now operate. Every workflow host needs its Dapr sidecar reachable and healthy, the placement service has to be there for the actor model underneath, and upgrading the runtime is a coordinated operation rather than a library bump. A workflow whose sidecar is missing does not fail loudly at compile time; it fails at start.
- It is the youngest of the three engines here, so pin the version and read that version's notes. The building block has moved through stability stages and its APIs and defaults have changed between releases, which makes an old sample an unreliable guide. Check the stability designation for the runtime version you are actually deploying.
