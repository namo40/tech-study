---
title: "Dapr Workflow"
summary: "Dapr Workflow は Dapr ランタイムの耐久ワークフローのビルディングブロックです。オーケストレーションは自分の言語の SDK で書き、進み具合はサイドカーが構成された状態ストアへ残し、再生に基づく実行のモデルは Durable Functions や Temporal と同じ系統のものです。"
...
- サービスがすでに Dapr で動いていて、そこへ長く走る過程が必要になったときに使います。ワークフローは状態管理や pub/sub の隣に並ぶビルディングブロックのひとつとして届き、同じサイドカーを通って届き、同じコンポーネントのファイルで構成され、同じアクセス制御で守られます。配置すべき二つ目のランタイムも、管理すべき二組目の資格情報も増えません。
- 自分たちの Kubernetes のクラスターの上で耐久のあるオーケストレーションを動かすときに使います。エンジンはホストされたサービスではなく自分のプロセスの隣のサイドカーで動きます。ですから再起動を越える複数段階の過程を、別のワークフローのサーバーを持ち込まずに自前の基盤で使えます。
- 問題の形がアクティビティとタイマーと外部のイベントと子ワークフローであるときに持ち出します。この四つの基本の要素が大半のオーケストレーションを覆います。ステップを呼んで耐久のある結果を受け取り、スレッドを抱えずに一日眠り、外から届く承認を待ち、大きな過程を互いを呼び合うワークフローへ分けます。
- 特定のクラウドのサーバーレスのオーケストレーターと、本格的なワークフローの基盤との中間として考えてみます。Durable Functions よりひとつのベンダーに縛られず、Temporal のクラスターを運用するより手間が軽く済みます。機能の深さより移植性が大事なときに筋の通った選択になる理由です。
...
- 状態ストアがそのまま耐久性であり、そのストアの保証がそのままワークフローの保証になります。進み具合は自分たちが構成したコンポーネントに住みます。ですからトランザクションに対応していない、バックアップがない、耐久性がない、そういうストアを選べばワークフローも同じになります。長く生きるインスタンスを安全とみなす前に、選んだストアがトランザクションとアクターの状態の要件に応えるかを確かめます。
- サイドカーはもう自分たちが運用するランタイムです。ワークフローのホストごとに Dapr のサイドカーが届いて健全でなければならず、その下のアクターのモデルのために placement のサービスも要り、ランタイムを上げることはライブラリの版を上げることではなく足並みをそろえる作業です。サイドカーのないワークフローは、コンパイルの時点で大きな音を立てて失敗するのではなく、起動のときに失敗します。
- ここに並ぶ三つのエンジンの中では最も若いので、バージョンを固定してそのバージョンのリリースノートを読みます。このビルディングブロックは安定性の段階を進んできましたし、リリースの間で API も既定値も変わりました。古いサンプルが当てにならない案内になる理由です。実際に配置するランタイムのバージョンの安定性の表記を確かめてください。
...
- インスタンスを始めることも、問い合わせることも、終わらせることも、メッセージではなくクライアントの呼び出しです。`DaprWorkflowClient` が自分で選んだ id で新しいインスタンスを予約し、現在の状態を読み、動いているインスタンスへ外部のイベントを差し入れ、終了させます。運用の道具や HTTP のエンドポイントが状態ストアに手を入れずにインスタンスを操る方法がこれです。
