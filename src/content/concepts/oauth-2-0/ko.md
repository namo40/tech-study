---
title: "OAuth 2.0"
summary: "OAuth 2.0은 비밀번호를 넘기는 대신 범위와 수명이 정해진 위임 토큰을 씁니다. 앱은 한동안 문 하나만 여는 열쇠를 받고, 회전은 토큰 탈취를 탐지 가능한 사건으로 바꾸며, OpenID Connect가 그 위에 \"누구인가\"에 대한 답을 얹습니다."
category: "인증과 인가"
scene: oauth-2-0
steps:
  - title: "집 전체의 열쇠를 넘기지 않습니다"
    text: "옛 방식에서 앱은 비밀번호를 요구했고, 그때부터 당신 이름으로 무엇이든 영원히 할 수 있었습니다. OAuth는 그것을 위임으로 바꿉니다. 당신이 승인하면 서버가 한동안 문 하나만 여는 열쇠를 발급하고, 앱은 비밀번호를 끝내 보지 못합니다."
  - title: "code는 영수증이고, 토큰이 열쇠입니다"
    text: "앱은 당신을 인가 서버로 보내고, 당신이 동의하면 1회용 code가 돌아옵니다. 앱은 그것을 read 범위의 access 토큰과 나중을 위한 refresh 토큰으로 교환합니다. access가 짧게 사는 것은 일부러 그런 것입니다. API는 사람이 아니라 토큰을 검사합니다."
  - title: "만료는 설계이고, 회전은 경보입니다"
    text: "access 토큰이 죽으면 API는 401을 말하고, refresh 토큰이 조용히 새것을 사 옵니다. 새 refresh 토큰도 함께 옵니다. 각각은 정확히 한 번만 통하기 때문입니다. 훔친 옛 refresh 토큰이 다시 사용되면 서버는 재사용을 알아채고 그 계열 전체를 폐기합니다. 도난은 영구적인 것이 아니라 시끄러운 것이 됩니다."
  - title: "OAuth는 무엇을 해도 되는가에 답하고, OpenID Connect는 누구인가에 답합니다"
    text: "다시 로그인하면서 앱은 openid 범위를 요청하고, access 토큰 옆에 id 토큰이 도착합니다. 신원에 대한 서명된 진술이고, 앱이 읽는 것이지 API로 보내는 것이 아닙니다. 질문 둘에 토큰 둘, 이제 둘 다 답이 있습니다."
related:
...
OAuth 2.0은 위임 프로토콜입니다. 딱 한 가지 상황을 위해 존재하고, 그 상황을 알아볼 수 있게 되면 나머지 설계는 거기서 따라 나옵니다. 어떤 소프트웨어가 사람을 대신해, 그 사람이 소유한 자원에 대해, 그 사람이 되지는 않으면서 동작해야 하는 상황입니다.
...
- 서버에서 렌더링하는 웹 앱이 사용자를 로그인시킨 뒤 API를 호출합니다. 앱이 정말로 client secret을 보관할 수 있다면 PKCE 없는 authorization code도 받아들일 만합니다. 그런 경우에도 PKCE는 비용이 들지 않고 주입 구멍 하나를 막아 줍니다.
...
- 토큰은 소지자 증서입니다. 만료되기 전까지, API 입장에서는 그것을 들고 있는 사람이 곧 당신입니다. 이 한 문장에서 규칙 대부분이 나옵니다. 짧은 수명, 모든 구간의 TLS, URL이나 로그 한 줄이나 분석 페이로드에 절대 넣지 않기, 크로스 사이트 스크립팅 결함이 닿을 수 있는 곳이라면 `localStorage`에도 넣지 않기입니다.
- access 토큰은 세션이 아닙니다. 만료는 있지만 로그아웃도, 유휴 시간 제한도, 만료 전에 믿고 쓸 수 있는 폐기도 없습니다. 애플리케이션의 로그인 상태를 access 토큰으로 짜면, 세션의 통제 수단은 하나도 없는 세션이 생깁니다.
- id 토큰을 API로 보내지 마세요. access 토큰에서 신원을 읽지도 마세요. id 토큰은 앱 앞으로, access 토큰은 API 앞으로 발행되고, 각각 자기 수신자를 대상으로 서명됩니다. id 토큰을 받아들이는 API는 다른 수신자용으로 발행된 토큰을 받아들이는 셈이고, `aud` 검증은 정확히 그 혼동을 막으려고 있습니다.
- refresh 토큰에는 회전과 재사용 감지가 필요합니다. 그것이 없으면 탈취된 refresh 토큰은 조용하고 영구적인 접근이 됩니다. 그것이 있으면 이미 쓴 토큰의 두 번째 사용이 신호가 되고, 올바른 대응은 토큰 하나가 아니라 계열 전체를 폐기하는 것입니다.
...
scope는 정책이 됩니다. 거친 문을 한 번 선언해 두고, 각 엔드포인트는 자기가 어느 문 뒤에 있는지만 말합니다.
...
사용자를 로그인시키는 웹 앱이라면 OpenID Connect 핸들러가 authorization code 흐름을 돌리고, PKCE는 기본으로 켜져 있습니다.
...
builder.Services
    ...
    .AddOpenIdConnect(options =>
    {
        ...
        options.Scope.Add("offline_access");     // refresh 토큰을 요청하는 것이 이 줄입니다
        options.Scope.Add("api://orders/orders.read");
    })
    .AddCookie();                                // id 토큰은 쿠키 세션이 됩니다
```

마지막 줄은 한 번 멈춰서 볼 만합니다. 핸들러는 id 토큰을 들고 있게 두지 않습니다. 한 번 검증해서 `ClaimsPrincipal`로 바꾸고 쿠키를 내려 줍니다. 그때부터 앱에는 세션이 있고, 토큰은 API를 호출하는 데 씁니다. 이것이 올바른 분업이고, 장면의 4단계가 그리는 것과 같은 분업입니다.

access 토큰을 받아 오고 캐시하는 일은 Microsoft.Identity.Web이 맡습니다. `ITokenAcquisition`이 refresh 토큰을 보관하고, 캐시된 access 토큰의 만료가 가까워지면 교환하고, 회전을 호출하는 쪽이 눈치채지 못하게 처리합니다.
...
이것이 운영에서 버티는지는 두 가지가 결정합니다. 토큰 캐시는 분산 캐시여야 합니다. 그렇지 않으면 스케일아웃한 앱은 요청이 다른 인스턴스로 갈 때마다 refresh 토큰을 잃고, 사용자는 보지 않아도 될 로그인 화면을 다시 봅니다. 그리고 client credentials 변형인 `GetAccessTokenForAppAsync`는 audience가 다른 별개의 승인 방식입니다. 사용자 토큰을 다루기 번거롭다고 요청 처리 중에 이쪽으로 손을 뻗으면, "이 사용자를 대신해 동작한다"가 "서비스 전체로서 동작한다"로 조용히 바뀝니다. OAuth가 막으려고 존재하는 바로 그 치환입니다.
