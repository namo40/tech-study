...
A memory limit is a number in a manifest and a kernel enforcement mechanism behind it. The container runtime writes the figure into a cgroup, and when the cgroup's memory — anonymous pages plus whatever page cache it cannot reclaim — goes over what the cgroup allows, the kernel picks a process inside it and sends it SIGKILL. There is no negotiation, no back-pressure and no grace period. The third step of the scene is that instant, and the only thing that happens in the application at that moment is that it stops.
