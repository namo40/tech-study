...
장면의 4단계는 폼에 칩을 하나 넣고 서버에 관문을 하나 세웁니다. 그 뒤로 우리 페이지에서 온 제출은 통과하고 위조된 쪽은 `400`으로 거부됩니다. 두 요청 모두 같은 세션 쿠키를 달고 도착하는데도 서버가 둘을 구별해 내는데, 이 지점은 정확히 짚어 둘 값어치가 있습니다. SameSite 혼자서는 할 수 없는 일이 바로 이것이기 때문입니다.
