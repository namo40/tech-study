...
summary: "Antiforgery token은 서버가 페이지 안에 심어 두고 다음 상태 변경 요청과 함께 돌려받기를 기대하는 값이며, 별도의 쿠키에 담긴 나머지 절반과 한 쌍을 이룹니다. 위조된 페이지는 브라우저가 쿠키를 보내게 만들 수는 있어도 당신 페이지를 읽지는 못하므로, 맞춰야 하는 절반을 되돌려 줄 수 없습니다."
category: "애플리케이션 보안"
...
장면의 네 번째 단계는 폼에 칩을 하나 넣고 서버에 관문을 하나 세웁니다. 그 뒤로 우리 페이지에서 온 제출은 통과하고 위조된 쪽은 `400`으로 거부됩니다. 두 요청 모두 같은 세션 쿠키를 달고 도착하는데도 서버가 둘을 구별해 내는데, 이 지점은 정확히 짚어 둘 값어치가 있습니다. SameSite 혼자서는 할 수 없는 일이 바로 이것이기 때문입니다.
...
실패 양상은 지루하고 흔한데, 그래서 적어 둘 값어치가 있습니다. 캐시된 페이지는 낡은 토큰을 들고 있어서 사용자가 새로 고칠 때까지 모든 제출이 실패하므로, 폼을 담은 페이지에는 `no-store`가 필요합니다. 폼을 아예 렌더링하지 않는 싱글 페이지 앱은 토큰을 따로 받아 헤더에 실어야 하고, 그 헤더 이름은 서버가 읽도록 설정된 것과 같아야 합니다. 세션이 만료된 뒤 제출된 폼은 로그인으로 리디렉션되는 대신 검증에서 실패하므로, antiforgery 예외를 알아보고 맨 `400` 대신 사용자를 적당한 곳으로 보내는 처리기가 필요합니다. 그리고 한 서버에서 렌더링된 토큰을 다른 서버에서 검증하면 data protection 키 링을 공유하지 않는 한 실패합니다. 이 키 링은 인증 쿠키가 의존하는 바로 그 키 링이고, 로드 밸런서 뒤에서 간헐적 실패로 드러납니다.
