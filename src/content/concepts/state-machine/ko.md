---
title: "State Machine"
summary: "상태 머신은 어떤 대상이 놓일 수 있는 상태와, 그 사이를 옮기는 이벤트를 이름 붙여 정리한 것입니다. 표에 없는 일은 일어날 수 없고, 현재 상태는 저장했다가 이어받을 수 있으며, 장기 실행 프로세스는 기다리고 만료되고 한 단계씩 재시도하는 기계가 됩니다."
...
steps:
  ...
  - title: "장기 실행 프로세스"
    text: "승인을 기다리고, 불안정한 단계를 재시도하고, 재시작을 견디는 프로세스는 내구성 있는 히스토리를 가진 상태 머신입니다. 워크플로 엔진은 그 히스토리를 재생해 상태를 다시 만들므로, 코드는 한 줄로 읽힙니다."
related:
...
- 전이는 반복해도 안전해야 합니다. 같은 이벤트가 두 번 도착해도 기계가 두 번 움직여서는 안 됩니다. 이벤트 id로 중복을 걸러 내거나, 핸들러가 떠나려는 상태를 확인하게 만듭니다.
- 상태 개수를 주시합니다. 상태 일곱에 이벤트 넷이면 표지만, 상태 마흔에 이벤트 서른이면 아무도 읽지 않는 그림입니다. 애그리거트별로 기계를 나누거나, 달라지는 부분을 데이터로 끌어올립니다.
- 인스턴스가 진행 중인데 워크플로 코드를 바꾸려면 버전 관리가 필요합니다. 히스토리를 재생해 상태를 세우는 엔진은 옛 히스토리를 새 코드로 다시 흘려보내므로, 이미 일어난 일의 모양이 새 코드에도 이해 가능한 채로 남아 있어야 합니다.
...
machine.Configure(OrderState.Submitted)
    .PermitIf(OrderTrigger.Pay, OrderState.Paid, () => payments.IsConfirmed(order.Id))   // 가드
    .Permit(OrderTrigger.Cancel, OrderState.Cancelled)
    .Permit(OrderTrigger.Timeout, OrderState.Expired);

machine.Configure(OrderState.Paid)
    .OnEntryAsync(() => mail.SendReceiptAsync(order.Id))                                  // 동작
...
await db.SaveChangesAsync(ct);   // 상태는 컬럼이고, 타이머는 만기 시각을 가진 행입니다
```

`CanFire`가 표를 두는 이유 그 자체입니다. 어떤 이벤트가 허용되는지 묻는 데 비용이 들지 않고, 그 답이 실제로 실행될 핸들러를 읽어 보는 일에 달려 있지 않습니다.

며칠씩 이어지는 프로세스라면 히스토리를 직접 관리하는 엔진에 기계를 맡깁니다. Azure Durable Functions, Temporal .NET SDK, Dapr Workflow는 모두 인스턴스에 이미 일어난 일을 재생해 상태를 다시 세웁니다. 그래서 오케스트레이션을 타이머나 외부 이벤트를 await하는 평범한 순차 코드로 쓰고도, 다른 머신에서 이어 실행할 수 있습니다. 단계가 하나의 프로세스가 아니라 여러 서비스에 걸쳐 있다면 사가로 잇고(.NET에서는 MassTransit 상태 머신이 흔한 형태입니다), 조정은 그 사가에 맡깁니다.
