...
In .NET the resources that run out are rarely the CPU. The thread pool, a `SemaphoreSlim` gate, `HttpClient`'s connection limit per endpoint, and the SQL connection pool are all fixed-size things with a queue behind them, and each one has a number you can read. Publish permits-in-use against permits-configured for anything you bound yourself, watch `threadpool-queue-length` and the pool counters for the ones the framework bounds for you, and treat any of them sitting near its limit as the same warning the scene draws: the ceiling is close, and the next thing that happens is the queue.
