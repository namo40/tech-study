...
So the test is not "does this migration apply cleanly" but a walk through three states, in the order the deploy actually uses them — schema first, then code. Apply the migration and deploy nothing at all: v1 still works. Deploy v2 against the migrated schema: v2 works. Roll v2 back: v1 still works. A change that passes all three is deployable one half at a time, which is what lets the schema step and the code step be two independent deploys instead of one coordinated event. Coordinated events are where outages live, because coordination is a promise about timing and rollouts do not make promises about timing.
