...
summary: "Materialized view는 비싼 쿼리의 결과를 실제 테이블로 저장해, 읽는 쪽이 스캔 대신 미리 계산된 행을 받게 합니다. 갱신 사이에는 낡은 값이 되므로, 어떻게 언제 갱신할지가 설계의 전부입니다."
category: "데이터 저장소"
scene: materialized-view
steps:
  - title: "매번 백만 행"
    text: "대시보드를 열 때마다 테이블 셋을 JOIN하고 백만 행을 집계해 30행을 만듭니다. 답은 1분 전과 같지만, 일은 매번 새로 합니다."
  - title: "답을 구체화합니다"
...
- 같은 집계나 JOIN을, 그 입력이 바뀌는 것보다 훨씬 자주 물어볼 때 씁니다. 한 시간에 두 번도 쓰기가 일어나지 않는 테이블을 열 명이 1분마다 대시보드로 여는 상황이 가장 분명한 예입니다.
...
- Materialized view는 설계상 낡은 값입니다. 대시보드의 숫자 옆에 갱신 주기를 함께 적습니다. 시각이 붙지 않은 숫자는 실시간 값으로 읽힙니다.
- 갱신은 진짜 일입니다. 한가한 시간대로 돌리고, 갱신 중에도 읽기가 막히지 않도록 concurrent나 증분 갱신을 쓰고, 쿼리를 보듯 갱신 시간도 지켜봅니다.
- SQL Server의 indexed view는 기본 테이블에 쓰기가 일어날 때마다 유지됩니다. 그 비용은 이득을 보는 읽는 쪽이 아니라 쓰는 쪽이 부담하는데, 장면의 마지막 단계가 말하는 거래가 바로 이것입니다.
- 전부 구체화하지는 않습니다. 쿼리 로그 상위 몇 개부터 시작하고, 나머지 긴 꼬리는 평범한 쿼리로 둡니다.
- 뷰는 인덱스의 대체품이 아닙니다. 받쳐 주는 인덱스가 없어서 쿼리가 느린 것이라면 인덱스를 먼저 넣고, 그러고도 구체화할 것이 남는지 봅니다.
...
GROUP BY 1;
CREATE UNIQUE INDEX ON sales_by_day (day);   -- REFRESH ... CONCURRENTLY에 필요합니다
```

EF Core는 이 뷰를 키 없는 엔티티로 매핑하므로, 읽는 쪽은 JOIN이 하나도 없는 30행짜리 평범한 `DbSet`이 됩니다. 갱신은 호스티드 서비스가 대시보드에 적어 둔 주기대로 맡습니다.
...
// 예약된 작업이 뷰를 갱신하고, 그동안 읽는 쪽은 옛 행을 계속 읽는다.
...
SQL Server는 장면의 증분 쪽을 다르게 씁니다. 뷰를 `WITH SCHEMABINDING`으로 만들고 고유 클러스터형 인덱스를 얹으면, 엔진이 기본 테이블에 쓰기가 일어날 때마다 뷰를 유지해 줍니다. 돌릴 갱신 작업도, 설명할 지연도 없습니다. 대신 `orders`에 들어가는 INSERT마다 집계의 일부를 함께 치르게 됩니다. 규칙 둘이 따라붙습니다. 그룹화하는 뷰는 `COUNT_BIG(*)`를 포함해야 하고 `HAVING`을 쓸 수 없으며, 옵티마이저가 뷰를 자동으로 대체해 주는 것은 Enterprise 에디션과 Azure SQL Database 또는 Managed Instance뿐입니다. Standard에서는 쿼리가 `WITH (NOEXPAND)`로 뷰를 직접 지명해야 합니다.
