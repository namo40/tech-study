...
summary: "비밀은 집이 하나여야 하고 나머지 모든 곳은 그 참조만 들고 있어야 합니다. 참조로 배포하면 교체가 릴리스가 아니라 데이터 변경이 되고, 모든 읽기를 감사해 두면 유출의 피해 범위를 실제로 잴 수 있습니다."
category: "인증과 인가"
scene: key-rotation
sceneStep: 4
related:
...
참조로 배포하는 방식이 그 대답을 가능하게 합니다. 애플리케이션에는 비밀의 이름과 그것을 읽을 수 있는 신원만 설정하고, 시작할 때와 갱신 주기마다 저장소에서 현재 값을 가져옵니다. 자격 증명 자체는 이미지에도 매니페스트에도 구워 들어가지 않으니 교체는 릴리스가 아니라 데이터 변경이 되고, 저장소가 바뀐 시점과 프로세스가 알아차리는 시점 사이의 간격은 풀 리퀘스트 대기열이 아니라 내가 고른 숫자가 됩니다. 그 숫자는 보안 파라미터입니다. 이미 값을 쥔 프로세스 안에서 폐기된 값이 계속 통하는 시간이 정확히 그만큼이기 때문입니다. 그러니 의도를 갖고 정하고, 비상 교체가 사고 처리 시간 안에 끝날 만큼 짧게 두고, 갱신에 실패하면 지난주 값으로 조용히 되돌아가는 대신 시끄럽게 실패하도록 만듭니다.
