...
シーンの 3 番目のステップは、ロールが尽きる場所です。`A` は editor なので、ロールは `A` が `B` の文書を直すことも快く許します。束には動詞が並んでいて、「自分のものだけ」は動詞ではないからです。ゲートをポリシーに切り替えると、同じリクエストが違う質問に出会います。「この呼び出し元はこのリソースの所有者か」という問いです。バッジもリクエストもそのままですが、いまはリソースが判断の一部であり、答えは拒否へ裏返ります。
...
構造で見ればポリシーは 1 つ以上の要件に付けた名前で、要件にはハンドラーが答えます。名前を付けるのは整頓以上の意味があります。規則の書き方が 1 つに定まるので、必要なすべてのエンドポイントで同じ一文が守られ、規則が Web サーバーなしで単体試験できる場所に置かれます。1 つの要件に複数のハンドラーが答えられ、たいていはそのうち 1 つが成功すれば十分です。「所有者または管理者」を、双方が互いを知らないまま表現できるのはそのおかげです。
...
よく出る失敗が 2 つあります。1 つは判断せずに終わるハンドラーです。沈黙は拒否でなければならないので、成功を告げ忘れた規則は断り、既定で成功する規則は名前の付いた穴になります。もう 1 つは、リクエストごとに、さらに一覧の項目ごとに小さなデータベース照会へ化けるポリシーです。規則にデータが要るなら、まとまり全体のために一度だけ読み、ハンドラーにはすでにメモリーにあるものを見せてください。そうしないと認可がそのまま性能の問題になります。
