...
steps:
  ...
  - title: "천천히 바꿉니다"
    text: "orders 트래픽의 10%를 새 코드로 보내고, 다음에는 절반, 그다음에는 전부로 늘립니다. 실패했을 때는 배포를 되돌리는 것이 아니라 경로만 되돌리면 됩니다."
  - title: "아무것도 남지 않을 때까지"
    text: "reports가 마지막으로 옮겨 가고 데이터도 함께 갑니다. 옛 시스템은 모든 줄이 new를 가리키는 라우터 뒤에서 퇴역합니다. 마이그레이션은 끝까지 합니다. 반만 옮긴 시스템은 양쪽의 단점만 남습니다."
related:
...
- 한 번에 다시 쓰기에는 너무 크거나 위험한 시스템을 교체할 때.
- 기능을 경로 뒤로 나눌 수 있고, 옮긴 뒤에는 기능마다 데이터 주인이 분명할 때.
- 옛 시스템을 다시 배포하지 않고도 기능 하나를 되돌릴 수 있어야 할 때.
