...
Bucket boundaries are therefore the precision control, and they are a design decision rather than a default to accept. A percentile lands inside a bucket rather than on a value, so it is reported as that bucket's edge or as an interpolation across it, and a bucket running from 100 ms to 500 ms can only ever say the answer is somewhere in there. Boundaries need to be dense where your target sits and may be sparse elsewhere. The explicit-bucket layout that `Histogram<T>` and the OpenTelemetry defaults give you is a fixed list of millisecond edges clustered under a second, which suits an HTTP handler and suits nothing else; exponential bucket histograms pick a scale automatically so the relative error stays constant across the range, which is what you want when the same instrument has to record 2 ms and 20 s.
