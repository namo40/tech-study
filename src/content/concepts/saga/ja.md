...
steps:
  ...
  - title: "pivot"
    text: "元に戻せないステップ、あるいは戻さないと決めたステップがあります。送ってしまったメール、返金しないと決めた確定済みの決済がそうです。そのステップを pivot と定め、その前では逆向きに補償し、その後では前向きに再試行します。すべてのステップは繰り返しても結果が同じでなければなりません。"
related:
  - label: Choreography
    slug: choreography
  - label: Orchestration
...
- 1 つの業務が、それぞれ自分のデータを持つ複数のサービスにまたがっていて、全体を 1 つの ACID トランザクションで包むことができない、または包みたくない場合。
...
- pivot を明示的に決め、順序の中でできるだけ後ろに置きます。その前では逆向きに補償し、その後では前向きの再試行だけを行います。送信済みのメールや、返金しないと決めた確定済みの決済に取り消しはないからです。
- メッセージは at-least-once で届くので、すべてのステップとすべての補償は saga id をキーにして二度実行されても安全、つまり冪等でなければなりません。
...
- 規模に合った形を選びます。Choreography はサービスを独立させる代わりに流れを隠し、Orchestration は流れを 1 か所に集める代わりにコーディネーターをクリティカルパスに置きます。
...
public sealed class OrderSaga : MassTransitStateMachine<OrderState>
{
    ...
    public OrderSaga()
    {
        ...
        // 宛先のない Send はエンドポイント規約で解決されるので、
        // EndpointConvention.Map<ReserveStock>(new Uri("queue:inventory")) を
        // 起動時に走らせておくか、Send(Uri, ...) のオーバーロードを使う。
        Initially(When(Submitted)
            .Send(ctx => new ReserveStock(ctx.Saga.CorrelationId))
            .TransitionTo(Reserving));

        // 戻せるステップを先に置くので、ここでの失敗は何も失わない。
        During(Reserving,
        ...
        // まだ pivot の手前: カードの拒否は確保を返すことで補償し、
        // saga は誰にも借りを残さず終わる。
        During(Charging,
            When(PaymentRefused)
            ...
            When(PaymentDone)
                .TransitionTo(Paid)                             // pivot: ここで金を受け取る
                .Send(ctx => new ConfirmOrder(ctx.Saga.CorrelationId)));

        // pivot を過ぎれば補償するものは残っていないので、分岐は前向きだけ。
        // 予算の中で再試行し、それから人に渡す。
        During(Paid,
            When(ConfirmFailed)
