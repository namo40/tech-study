...
- p95는 치솟는데 p50은 그대로일 때 이 글을 읽을 만합니다. 중앙값은 멀쩡한데 꼬리만 나빠지는 모양은 공유 리소스 앞의 줄이 만들어 내는 전형적인 그림입니다. 클라이언트가 HTTP/1.1이고 무언가가 경로를 연결 하나에 묶어 두었다면(오리진당 한도에 닿은 브라우저, `MaxConnectionsPerServer`를 낮게 잡은 `HttpClient`, 연결 하나만 붙드는 프록시 구간), 같은 호스트로 가는 작은 호출 여덟 개 중 여덟 번째는 서버가 바쁘든 한가하든 앞의 일곱 개를 기다립니다.
...
- 서비스 사이 호출에서는 특히 진지하게 봐야 합니다. 거기서 쓰는 임시방편은 소켓으로 대가를 치르기 때문입니다. 브라우저는 오리진당 연결 여섯 개를 열어 HTTP/1.1의 문제를 오랫동안 가려 왔는데, 그것은 하나 안에서 얻어 낸 병렬성이 아니라 연결로 사들인 병렬성입니다. `HttpClient`도 같은 방식으로 사들이되 천장이 아예 없습니다. `MaxConnectionsPerServer`가 기본값으로 무제한이라, 여덟 개짜리 팬아웃은 연결 여덟 개를 열고 비용은 줄이 아니라 핸드셰이크와 `TIME_WAIT`에 쌓인 소켓으로 나타납니다. 그 천장을 두는 순간 줄이 돌아옵니다.
...
- 느린 서버를 고치려고 꺼내지는 않습니다. 다중화가 없애는 것은 요청이 다른 요청을 기다리는 시간입니다. 요청이 답을 만드는 데 쓰는 시간에는 아무 영향도 주지 않습니다. 장면 속 느린 호출이 1초 걸린 이유가 뒤의 쿼리가 1초 걸리기 때문이라면, 장면의 어느 단계도 그 1초보다 일찍 끝나지 않습니다.
...
- 나와 서버 사이에 무엇이 있는지 확인합니다. 로드 밸런서나 리버스 프록시, 서비스 메시 사이드카가 HTTP/2를 종료하고 그다음 구간을 HTTP/1.1로 말한다면, 다중화는 엣지까지고 그 뒤에는 줄이 있습니다. 그것이 반드시 틀렸다는 뜻은 아닙니다. 엣지 뒤의 구간은 대개 짧고 거기 연결 풀은 큽니다. 다만 그 성질이 원본 서버까지 닿는다고 가정하는 대신 어디서 끝나는지 알아 두는 편이 좋습니다.
- 연결이 줄어들면 장애 양상과 부하 분산도 달라집니다. 하나가 전부를 실어 나른다는 말은 그 하나를 잃으면 전부를 함께 잃는다는 말이고, 레이어 4(L4) 로드 밸런서는 요청이 아니라 연결을 나누므로 오래 사는 다중화 연결은 자기 트래픽 전부를 처음 붙은 백엔드에 고정합니다. 둘 다 대개 받아들일 만하지만, 아무도 소리 내어 말하지 않으면 둘 다 놀라움이 됩니다.

## .NET에서는

- 클라이언트 쪽 동작은 `SocketsHttpHandler`에 있습니다. 버전과 정책을 요청이나 클라이언트에 지정하되, `HttpVersion.Version20`만으로는 뒤로 물러나는 선호값에 그친다는 점을 기억합니다. 기본 정책이 `RequestVersionOrLower`이기 때문입니다. 아래에서 쓴 `RequestVersionOrHigher`는 HTTP/2 아래로는 내려가지 않되 서버가 HTTP/3을 광고하면 핸들러가 그쪽을 택하도록 허용하고, `RequestVersionExact`는 HTTP/2 하나로 고정합니다. 어느 쪽이든 협상 실패는 조용히 줄로 내려앉는 대신 오류가 됩니다.
...
- gRPC에서는 주소마다 `GrpcChannel`을 하나 만들어 계속 씁니다. 채널이 연결을 쥐고 있고 그 위의 모든 호출은 스트림이며, 중요한 동시성 한도는 호출하는 코드가 아니라 서버의 `MaxStreamsPerConnection`입니다. 부하 테스트가 의심스럽게 동그란 동시 호출 수에서 평평해진다면 그 설정부터 봅니다.
