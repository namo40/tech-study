...
.NET には出来合いの Retry Budget 戦略がありません。Polly にも `Microsoft.Extensions.Http.Resilience` にも同梱されていないので、選択肢は、いちばん近い代替としての Circuit Breaker、Budget を実装してくれるサービスメッシュ、あるいは `OnRetry` で通常トラフィックに対する再試行を自分で数えることのいずれかです。
