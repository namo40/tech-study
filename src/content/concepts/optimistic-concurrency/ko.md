...
summary: "낙관적 동시성은 잠금을 쥐지 않습니다. 행을 버전과 함께 읽고, 일을 한 뒤, 버전이 그대로일 때만 씁니다. 충돌은 저장 시점에 드러나고, 작업 단위 전체를 다시 실행합니다."
category: "트랜잭션과 동시성"
scene: deadlock
sceneStep: 4
related:
...
이 방식이 값어치를 하는 곳은 읽기와 쓰기 사이의 시간입니다. 사용자가 편집 화면을 열고 2분을 고민한 뒤 저장을 누른다고 해 봅시다. 비관적 잠금이라면 행 하나에 배타 잠금을 2분 동안 걸어 두는 것이고, 많은 구성에서는 빌린 연결도 2분 동안 붙들고 있는 것입니다. 버전 검사라면 쥐고 있는 것이 없습니다. 읽는 요청과 쓰는 요청은 서로 분리되어 짧게 끝나고, 세상이 바뀌었는지는 두 번째 요청이 저장 시점에 알게 됩니다.
...
for (var attempt = 0; attempt < 3; attempt++)
{
    ...
    {
        // 아무것도 쓰이지 않았다. 낡은 추적 정보를 버려 루프의 FindAsync가
        // 현재 행을 읽고 그 행에 `delta`를 다시 적용하게 한다.
        db.ChangeTracker.Clear();
    }
}
```

만능은 아닙니다. 낙관적 동시성은 행 하나를 경쟁하는 쓰기로부터 지켜 줄 뿐, 두 행을 서로 맞춰 주지는 않고, 충돌이 갱신이 아니라 삽입일 때는 도움이 되지 않습니다. 충돌이 이따금이 아니라 늘 일어난다면 재시도 자체가 비용이 되므로, 짧은 비관적 잠금이 더 쌉니다.
