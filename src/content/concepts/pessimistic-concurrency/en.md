...
The bet is in the name. Pessimistic concurrency assumes a second writer is coming, so it takes an exclusive lock at read time and keeps it until the commit. Nobody else can change the row in between, which means the value the transaction read is still the value it is updating. Where optimistic concurrency detects a conflict afterwards, this prevents it from happening at all.
