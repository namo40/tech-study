...
steps:
  ...
  - title: "Retry-After"
    text: "429 応答には、いつ戻ればよいかのヒントが含まれます。それを守るクライアントは補充のあとに戻ってきて、バーストは一定の流れに変わります。"
  - title: "パーティション"
    text: "制限はクライアント、テナント、エンドポイントなどのキーごとに設けます。騒がしい呼び出し元は自分のバケットだけを空にします。インスタンスが複数あるなら、バケットは共有ストアに置きます。"
related:
  - label: Token Bucket
    slug: token-bucket
  - label: Leaky Bucket
...
- 公開 API や複数の利用者が共有する API で、1 つの呼び出し元がほかを押しのけられる場合
...
- キーは慎重に選びます。IP で制限すると NAT の内側にいる利用者がまとめて巻き添えになります。API キーや利用者単位で制限するほうが、たいていは公平です。
