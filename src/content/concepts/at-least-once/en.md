...
In .NET this is what the Azure Service Bus receive modes are about. `ReceiveMode.PeekLock` is the at-least-once path: `CompleteMessageAsync` after the work, `AbandonMessageAsync` to give it back straight away, `RenewMessageLockAsync` when the handler legitimately needs longer, and a lock that simply expires if the process disappears. `ReceiveMode.ReceiveAndDelete` is the at-most-once path, and it is the right choice only where losing a message costs less than handling it twice, such as a telemetry stream. RabbitMQ draws the same line with `autoAck`: leave it off, acknowledge after the work, and let `basic.nack` with requeue hand a message back deliberately.
