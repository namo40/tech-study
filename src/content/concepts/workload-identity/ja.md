...
steps:
  ...
  - title: "すべてのワークロードは、ちょうど自分の分だけを握ります"
    text: "pod A の role はストレージを開き、pod B の role はデータベースを開きます。どちらも相手の権限を借りられません。共用サービスアカウントがないので、侵害されたポッドが漏らすのは艦隊全体ではなく、そのワークロードの権限だけです。アイデンティティがここまで細かければ、最小権限は書類仕事であることをやめます。トークンが元からそう出てくるだけです。"
related:
