...
代償は、baggage が宛先を指定した配信ではなく放送だという点にあります。コンテキストが届くすべてのホップがそれを見ます。他のチームが持つサービスも、外部呼び出しにまで伝播させるなら第三者も含まれます。項目一つ一つがその trace のすべてのリクエストでバイトを占め、標準は全体の大きさにあえて控えめな上限を置いています。したがって守るべき規則は短いものです。キーは 2 つか 3 つに抑え、値は小さく範囲の閉じたものにし、秘密や個人情報は決して入れず、自分のシステムが終わる境界で取り除いてください。
...
.NET では `Baggage.SetBaggage("tenant", value)` と `Baggage.GetBaggage("tenant")` が API で、項目は `Activity.Current` と同じように非同期コンテキストを流れます。回線上の伝播は複合プロパゲーターが担い、baggage を trace コンテキストと一緒に運ぶので、HTTP のホップにコードは要らず、メッセージのホップには他のコンテキストと同じ inject と extract が要ります。`Activity` にも `AddBaggage` があり、同じ考えを BCL の水準で先に書いた綴りですが、2 つは 1 つの保管場所ではありません。OpenTelemetry のプロパゲーターが inject するのは `Baggage.Current` だけなので、`Activity.AddBaggage` で足した項目は回線に届きません。OpenTelemetry API の README も明言しています。baggage を足す推奨の方法は `Baggage.SetBaggage()` であり、OpenTelemetry のユーザーは `Activity.AddBaggage` を使うべきではありません。
