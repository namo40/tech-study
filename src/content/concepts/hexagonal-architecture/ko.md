...
summary: "헥사고날 아키텍처는 모든 의존이 안쪽을 향하게 합니다. 도메인은 가운데에서 바깥 이름을 하나도 모른 채 앉아 있고, 포트는 그 도메인이 말하는 계약이며, 어댑터는 세상을 그 계약으로 번역합니다. 계약이 곧 경계이기 때문에 웹도 데이터베이스도 테스트 하네스도 갈아 끼울 수 있는 세부 사항이 됩니다."
category: "애플리케이션 아키텍처"
scene: hexagonal-architecture
steps:
  - title: "방향이 없으면 바깥이 안쪽에 금을 냅니다"
    text: "고스트는 데이터베이스에 곧장 배선된 도메인을 보여 줍니다. 컬럼 이름 하나가 바뀌자 도메인에 금이 가고, 그 금은 웹 계층까지 번집니다. 누가 고른 것이 아니라, 아무도 방향을 정해 주지 않을 때 의존성이 하는 일입니다. 이 아키텍처 전체가 규칙 하나입니다. 모든 의존은 안쪽을 향한다."
  - title: "안쪽은 계약만 말합니다"
    text: "포트는 도메인이 소유한 인터페이스입니다. 요청은 이쪽 포트로 들어오고, 도메인의 필요는 저쪽 포트로 나가게 됩니다. 둘 다 도메인의 언어로, \"이 주문을 저장하라\"이지 \"INSERT INTO\"가 아닙니다. 웹도, SQL도, 벤더 이름도 없습니다. 그것이 설계입니다."
  - title: "어댑터는 세상을 계약으로 번역합니다"
    text: "한쪽 면으로는 HTTP나 SQL을, 다른 면으로는 포트만 말합니다. 데이터베이스 어댑터를 메모리 어댑터로 바꿔도 도메인은 모릅니다. 그래서 이 장면의 테스트는 데이터베이스 없이 진짜 도메인을 전속력으로 돌립니다. 바꿀 수 있었다면 세부 사항이고, 어댑터는 세부 사항이 모이는 곳입니다."
  - title: "같은 규칙, 다른 그림들입니다"
    text: "카드 위의 세 이름은 한 규칙을 그린 세 그림입니다. 헥사고날, 클린, 어니언. 의존은 안쪽을 향하고, 가운데는 바깥 이름을 모릅니다. 팀이 좋아하는 그림을 고르고, 규칙을 지키고, 논쟁은 다른 곳에 씁니다."
related:
...
- 아무것도 띄우지 않고 핵심을 테스트하고 싶을 때입니다. 포트에 가짜 어댑터를 꽂으면 진짜 규칙을 메서드 호출 속도로 검증하는 테스트가 됩니다. 데이터베이스도, HTTP 호스트도, 컨테이너도 필요 없습니다. 장면의 3단계가 바로 그 맞바꿈입니다. 같은 도메인, 같은 소켓에 꽂힌 다른 물건, 그리고 몇 분의 일로 줄어든 응답 시간입니다.
- 한 애플리케이션이 여러 바깥을 동시에 마주할 때입니다. 같은 일을 해야 하는 HTTP 엔드포인트와 큐 컨슈머와 야간 스케줄러는 하나의 포트 위에 놓인 어댑터 셋이지, 서로 다른 버그를 하나씩 품은 로직 사본 셋이 아닙니다. 들어오는 길이 둘째로 늘어나는 순간 이 구조는 제값을 합니다.
...
- CRUD만 얇게 얹은 서비스에는 **쓰지 않습니다**. 요청이 "이 필드들을 저 테이블에 쓰라"이고 도메인에 지킬 규칙이 없다면, 포트와 어댑터는 지켜 주는 것 하나 없이 홉과 인터페이스와 매핑만 더합니다. 통과만 하는 도메인이 얻는 것은 형식적인 절차뿐입니다. 그럴 때는 프레임워크를 직접 쓰고, 지켜 낼 것이 있는 곳에 노력을 쓰는 편이 낫습니다.
...
- 계층을 계층 자체를 위해 늘리지 않습니다. 가치는 의존의 방향에 있지 고리의 개수에 있지 않습니다. 규칙 하나가 분명한 프로젝트 넷이 아무도 다시 말하지 못하는 규칙을 가진 프로젝트 아홉보다 낫고, 호출을 그냥 넘기기만 하는 계층은 누군가 급해지는 첫 순간에 건너뛰어집니다.
- 경계마다 붙는 매핑이 눈에 보이는 비용입니다. 요청 모델에서 커맨드로, 커맨드에서 도메인 객체로, 도메인 객체에서 영속 모델로, 그리고 다시 되돌리는 코드는 데이터베이스에 직접 붙는 설계라면 없었을 진짜 코드입니다. 보호가 실재하는 곳에서는 그 대가를 치르고, 그렇지 않은 곳에서는 거절합니다. CRUD 서비스가 이 구조를 채택하면 안 되는 정직한 이유가 여기에 있습니다.
- 컴포지션 루트는 모든 화살표가 마침내 만나는 자리이고, 전부를 알아도 되는 유일한 곳입니다. 작게 유지하고 가장 바깥 프로젝트에 두며, 핵심 안쪽에서 서비스 로케이터에 손을 뻗고 싶은 유혹은 물리쳐야 합니다. 컨테이너에 물건을 달라고 할 수 있는 안쪽은 다시 바깥 의존을 가진 것이고, 그것도 눈에 보이지 않는 형태로 가진 것입니다.
...
```
Shop.Domain           <- 참조 없음. 엔티티, 값 객체, 그리고 포트들
...
`IClock`도 포트이고, .NET이 이미 제공하는 것이 있습니다. `TimeProvider`가 그 내장판으로, `TimeProvider.System`으로 등록하고 테스트에서는 `Microsoft.Extensions.TimeProvider.Testing`의 `FakeTimeProvider`로 대체합니다. 도메인이 시간에 대한 자기 어휘를 원할 때는 인터페이스를 직접 쓰고, 그렇지 않을 때는 내장된 것을 씁니다.
...
// 인프라 프로젝트가 자기 어댑터를 직접 등록한다. 어댑터가 internal로 남을 수 있는
// 이유이고, 다른 프로젝트는 그 이름을 볼 필요가 없다.
...
컴포지션 루트는 양쪽 이름을 함께 부르는 유일한 곳입니다. 다만 바깥쪽에서 부르는 이름은 어댑터 클래스가 아니라 인프라 프로젝트이고, 그래서 `EfOrderStore`가 `internal`로 남을 수 있습니다. 그 위의 모든 코드는 인터페이스를 상대로 쓰였고, 그 인터페이스에 마침내 몸이 주어지는 자리가 여기입니다.

```csharp
// Shop.Web의 Program.cs
builder.Services.AddInfrastructure(connectionString);      // 구동되는 쪽 어댑터들
