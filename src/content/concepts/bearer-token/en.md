...
steps:
  ...
  - title: "Readable, not secret"
    text: "A JWT is three base64 parts: header, payload — the claims — and signature. Anyone holding it can read them: who it is for, what it allows, when it dies. Encoding is not encryption; the signature only proves nothing was changed."
  - title: "Validation gates, and the leak"
    text: "The API checks signature, expiry and audience: cheap, local, no issuer call. Then it leaks, and the attacker passes the same gates: possession is authority. The expiry caps the damage — for the attacker, and for the client until it reissues."
