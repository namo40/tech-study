...
steps:
  ...
  - title: "N에 비례해 커집니다"
    text: "같은 코드가 행 20건에서는 왕복 21번을 하고, 시간 막대는 끝을 넘어갑니다. 개발 데이터는 이것이 드러날 만큼 행이 많지 않고, 운영 데이터는 많습니다."
  - title: "Include"
    text: "연관된 행을 같은 쿼리에서 함께 요청하면 데이터베이스가 JOIN으로 묶어 줍니다. 왕복 한 번에 모든 데이터입니다. 컬렉션을 여러 개 Include할 때는 카테시안 곱 폭발을 조심합니다."
  - title: "필요한 것만 읽습니다"
    text: "Select로 프로젝션해 쓰는 컬럼만 오가게 하거나, 조회를 IN 쿼리 하나로 묶습니다. 그리고 쿼리 로그를 읽습니다. 왕복을 숨기는 ORM이 진짜 문제입니다."
related:
...
- EF Core 로그에서 페이지 크기에 따라 함께 늘어나는 쿼리 수입니다.
- 로컬에서는 빠른데 운영에서는 느린 엔드포인트. 이때 데이터베이스의 CPU는 낮고 요청 수는 높습니다.

## 주의점

- `Include`도 공짜는 아닙니다. 컬렉션을 여러 개 Include하면 행이 곱해져 카테시안 곱 폭발이 일어나므로, `AsSplitQuery`를 쓰거나 프로젝션으로 대신합니다.
- 지연 로딩 프록시는 탐색 속성에 접근할 때마다 쿼리를 냅니다. 즉시 로딩이나 프로젝션을 우선하고, 정말로 그때그때 불러와야 한다면 명시적으로 불러와서 비용을 치르는 코드에서 그 쿼리가 보이게 합니다.
- 읽기 전용 엔드포인트에는 `Select` 프로젝션이 대개 가장 좋은 답입니다. 컬럼이 적고, 변경 추적이 없고, 쿼리가 하나입니다.
- 테스트에서 쿼리를 로그로 남기고 개수를 셉니다. 목록 엔드포인트에는 그 개수를 단언합니다.

## .NET에서는

```csharp
// N+1: 목록에 쿼리 하나, 그다음 행마다 하나씩.
var orders = await db.Orders.ToListAsync(ct);
foreach (var order in orders)
{
    var customer = await db.Customers.FindAsync([order.CustomerId], ct); // 주문마다 왕복 한 번
    Console.WriteLine($"{order.Id}: {customer!.Name}");
}

// Include: JOIN이 들어간 쿼리 하나.
var withCustomers = await db.Orders
    .Include(o => o.Customer)
    .ToListAsync(ct);

// Select: 필요한 컬럼만, 추적 없이, 쿼리 하나.
