...
steps:
  ...
  - title: "ヘルスチェック"
    text: "probe が 2 回失敗したサーバーはローテーションから外し、2 回成功したら戻します。故障から 2 回目の probe までのあいだは 2 件のリクエストがまだ失敗するので、probe の間隔が重要です。"
  - title: "スケールアウト"
...
- ドレインは追加と同じくらい重要です。降ろすインスタンスにはまず新しい仕事を送るのをやめ、抱えているものを終わらせるのは後にします。そうしないとデプロイがエラーの山に変わります。
...
```csharp
// アプリ側。トラフィックを受けるのに必要なものを確かめる readiness probe です。
...
`LeastRequests` が YARP の least connections で、しきい値 2 の `ConsecutiveFailures` (YARP 自身の既定値ですが、分かりやすいように上で明示しました) は、シーンが描いている規則の失敗側の半分です。probe の 1 回の失敗はノイズ、連続 2 回は判断です。復帰の側では YARP のほうがシーンより単純で、probe が 1 回成功した時点で宛先を健全に戻します。シーンのような成功回数のしきい値は、YARP ではなく Kubernetes の `successThreshold` に近い設定です。Application Gateway や Kubernetes の Ingress のような L7 のロードバランサーは、同じ 3 つの部品で設定します。宛先を選ぶポリシー、宛先の一覧を決めるヘルスチェック、そして背後のアプリが元のリクエストを見られるようにする転送ヘッダーです。Azure Load Balancer や Kubernetes の Service のような L4 のものには最初の 2 つしかなく、Service ではその 2 つ目も独自の probe ではなくポッドの readiness です。リクエストを読まないので、足すべきヘッダーがないからです。
