...
- Choose it when the broker has to run where you run and belong to nobody else. It installs on a server, in a container or in a Kubernetes cluster, it speaks AMQP 0-9-1 so the clients are not tied to a vendor, and moving from a laptop to a data centre to a different cloud is the same broker with a different address.
