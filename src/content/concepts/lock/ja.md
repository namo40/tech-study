...
アプリケーションが目にするロックは、2 つのモードでほぼ説明が付きます。SQL Server の既定であるロック方式の read committed では、共有ロックは読み取りが取るロックで、同じ行に複数の読み取りが同時に取れます。排他ロックは書き込みが取るロックで、何とも同居できません。排他ロックがかかっているあいだ、ほかのトランザクションはその行を共有ロックで読むことも、書くこともできません。「2 つ目の書き込みが待つ」という言い方の中身がこれです。データベースが読み取りを行バージョンから答える場合 (PostgreSQL は常に、SQL Server は `READ_COMMITTED_SNAPSHOT` を有効にした後) は、読み取りは共有ロックを取らず書き手に妨げられることもなく、この絵のうち書き込みの側だけが残ります。
...
ロックには大きさもあります。データベースは 1 行をロックすることも、ページをロックすることも、テーブル全体をロックすることもでき、たとえば SQL Server は、1 つの文が十分に多くのロックを取ると、小さなロックの束を 1 つの大きなロックへ昇格させます。支えるインデックスのない更新はテーブルを走査し、通り道の行にロックを取っていき、実際に変更した行よりはるかに広い範囲を握ることになります。足りないインデックスが、遅いクエリではなくブロッキングの問題として現れる理由です。
