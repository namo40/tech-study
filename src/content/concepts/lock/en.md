...
Two lock modes cover most of what an application sees. A shared lock is what a read takes, and several readers can hold one on the same row at once. An exclusive lock is what a write takes, and it is compatible with nothing: while it is held, no other transaction may read the row under a shared lock or write it at all. That is the whole mechanism behind "the second writer waits".
...
Locks also have a size. A database can lock a row, a page, or a whole table, and it will escalate from many small locks to one big one when a statement touches enough of the table. An update with no supporting index scans, takes locks along the way, and can end up holding far more than the rows it changed, so a missing index shows up as a blocking problem rather than a slow one.
