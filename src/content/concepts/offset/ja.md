---
title: "Offset"
summary: "共有ログに挟んだしおりです。1 つの購読がどこまで読んだかを示す位置で、ログではなく読む側が持ちます。だから同じイベントを二人の読者が別々の場所で読んでも、どちらも相手を止めません。"
...
offset はしおりであり、シーンの 2 番目のステップはそれを誰が持つのかという話です。ログはイベントを八つ抱えたまま、どれも手放しません。購読が読んだときに変わるのはログではなく、その購読の名前の下にある数字と、セルの一つの下に置かれた棒です。A は最新のイベントと並び、B は二つ遅れ、二つのしおりは同じログの違う場所に同時に挟まっています。配信が何も取り出さないからこそ成り立ちます。

ここがトピックとキューの分かれ目で、正確に言っておく値打ちがあります。ワークキューでは状態をブローカーが持ちます。誰かが処理しているあいだメッセージはロックされ、ack すれば消えるので、キューの中身はコンシューマーが何をしたかで決まります。ログではブローカーはイベントだけを持ち、位置はコンシューマーがそれぞれ持ちます。B の位置はログのどこにも現れず、A の位置は B の中に現れないので、キューでよくあるあの失敗、つまり遅いコンシューマー 1 つがロックの後ろに全員を並ばせることが起きる場所そのものがありません。代償は、残りの仕事量をブローカーが教えてくれなくなることです。その値はログの先頭から offset を引いて求めるもので、通知を仕掛けるべき値もそれです。
...
このしおりが説明で終わらず役に立つのは、二つの性質のおかげです。前にしか動かないので、配信は常に次のイベントであって飛躍ではありません。そして一度通り過ぎた場所は、誰かが意図して巻き戻さないかぎり通り過ぎたままです。もう一つは動かす値段の安さで、動かす作業がブローカーへのメッセージではなく算術だからです。安くないのは再起動を越えて生き延びさせるほうで、それは名前の違う別のものです。checkpoint とは、どこか durable な場所に書き留めた offset のことです。シーンで二つをわざと離して描いた理由がここにあります。B の offset は B のメモリにあり、B の checkpoint はそうではなく、その差こそ 3 番目のステップが語るものです。
