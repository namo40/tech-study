...
steps:
  - title: '예산은 "내려라"를 "한 번에 하나씩"으로 바꿉니다'
    text: "드레인은 파드 둘을 내리고 싶어 하고, 예산은 둘이 준비 상태여야 한다고 요구합니다. 그래서 축출은 정확히 하나만 진행되고 다른 하나는 기다립니다. 거절이 아니라 보류입니다. 유지보수가 장애 대신 줄서기가 됩니다."
  ...
  - title: "예산이 실제로 사 준 것입니다"
    text: "교대 파드가 올라와 준비 수가 셋으로 돌아오고, 그제야 두 번째 축출이 시작됩니다. 드레인 전 과정에서 ready는 둘 아래로 내려간 적이 없고 SLO 램프는 한 번도 깜빡이지 않았습니다. 그것이 제품의 전부입니다. 사용자가 감지할 수 없는 계획 작업."
related:
...
- 레플리카 수가 처리량이 아니라 가용성을 위해 존재하는 Deployment라면 씁니다. 하나를 잃어도 견디려고 사본을 셋 두었다면, 플랫폼이 셋을 한꺼번에 가져가는 것을 막아 주는 것은 예산뿐입니다. Kubernetes의 다른 어떤 것도 그 파드 셋이 같은 약속의 사본 셋이라는 사실을 알지 못합니다.
...
- 예산은 준비된 파드 개수에 대한 산술이고, readiness가 거짓말을 하면 그 산술은 쓸모가 없습니다. 연결을 거부하면서도 readiness 프로브에 200을 돌려주는 파드는 `minAvailable`에 포함되지만 누구에게도 아무 도움이 되지 않습니다. 예산이 지키는 모든 것은 프로브가 "이 파드가 지금 요청을 처리할 수 있는가"에 정직하게 답한다는 전제 위에 서 있습니다.
- 파드는 삭제되는 순간 endpoints에서 빠지지만, 그 사본이 갱신되는 데는 시간이 걸립니다. 이 파드를 지목하는 엔드포인트 목록은 클러스터의 모든 노드에 있으므로, "이 파드는 곧 사라진다"와 "이제 아무도 나에게 라우팅하지 않는다" 사이에는 시차가 생깁니다. preStop 훅은 바로 그 시차에 앉아 있으라고 있습니다. 이 훅을 건너뛰면 트래픽이 아직 도착하는 동안 파드가 수신을 멈추고, 그것은 정상 종료로 포장한 502입니다.
...
- 레플리카가 하나인 워크로드에 예산을 붙여 두고 도움이 되기를 기대하지는 않습니다. 레플리카 하나에 `minAvailable: 1`이면 모든 드레인이 막히고, 하나에 `maxUnavailable: 1`이면 존재하는 유일한 중단을 허용합니다. 예산은 레플리카 수가 갖지 못한 가용성을 만들어 내지 못하며, 그런 척하면 장애가 파드에서 업그레이드로 옮겨 갈 뿐입니다.
...
호스트가 모르는 것이 하나 있는데, readiness 엔드포인트입니다. 예산이 읽는 것이 바로 그 부분입니다. 예산은 `Ready` 조건이 참인 파드를 세므로, 파드가 도는 동안의 산술은 프로브만큼만 정확합니다. 종료가 시작되는 순간에도 readiness를 실패시켜서, 이 파드를 직접 프로브하는 라우터가 일을 보내지 않게 합니다. Kubernetes의 Service endpoints는 삭제 시점에 이미 통보를 받았습니다.

```csharp
// readiness 프로브가 찌를 대상으로 등록합니다. 이 엔드포인트는 "나에게 라우팅해도
...
public sealed class OutboxPump(IHostApplicationLifetime lifetime) : BackgroundService
{
    protected override async Task ExecuteAsync(CancellationToken stopping)
    {
        try
        {
            ...
            {
                // 이 토큰은 SIGTERM에서 호스트가 취소합니다. 토큰을 무시하는
                // 루프는 유예 시간이 쓰기 도중에 죽여 버리는 루프입니다.
        ...
        {
            // 취소는 거의 언제나 delay 안에서 일어나고, 예외로 도착합니다.
            // 이것이 없으면 아래의 drain은 결코 실행되지 않습니다.
        }
...
플랫폼 쪽에서 예산은 네 줄이고, 절차의 시간 값이 정해지는 곳은 파드 스펙입니다. preStop의 sleep은 꼼수가 아닙니다. 엔드포인트 목록은 파드가 삭제되는 순간부터 이 파드를 빼기 시작했고, sleep은 그 사본들이 따라잡는 시차 구간입니다.
...
metadata:
  name: api
spec:
  minAvailable: 2          # 레플리카 셋 중 둘, 그래서 한 번에 하나씩 축출
  selector:
    matchLabels:
      app: api
---
      ...
      containers:
        - name: api
          lifecycle:
            preStop:
              exec:
                # 삭제 시점에 이 파드의 엔드포인트는 이미 terminating으로 표시되었습니다.
                # 이 멈춤은 리스너를 닫기 전에 모든 라우팅 테이블이 그 사실을 듣게 하는
                # 시간이고, 그 안에서 다른 일은 아무것도 일어나지 않습니다.
...
세 숫자는 함께 읽어야 합니다. endpoints 변경은 삭제 시점에 시작되고, 훅은 그것이 전파되는 동안 파드를 10초 붙잡고, 호스트는 최대 25초 동안 마무리하고, 플랫폼은 45초까지 기다립니다. readiness가 503으로 바뀌는 것은 훅이 끝나 SIGTERM이 도착할 때이고, 파드를 직접 프로브하는 라우터에는 그것으로 충분히 이릅니다. 훅이 도는 동안에도 실패시켜야 한다면 훅이 자는 대신 애플리케이션에 신호를 주어야 합니다. 애플리케이션이 지켜보는 파일을 건드리거나 로컬 엔드포인트를 부르는 식입니다. 하나를 바꾸면 나머지 둘을 확인합니다. 잘못 잡았을 때의 유일한 증상은 배포 중에만 실패하고 다른 어디에서도 실패하지 않는 소수의 요청이기 때문입니다.
...
# 뺀 값이고 계속 다시 계산되며, 축출하는 컨트롤러가 무언가를 가져가기 전에
# 읽는 값입니다.
