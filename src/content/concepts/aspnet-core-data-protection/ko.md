...
summary: "Data Protection은 프레임워크가 쿠키와 antiforgery 토큰과 TempData를 보호할 때 직접 쓰는 암호화 API입니다. `IDataProtector`가 페이로드를 암호화하고 무결성을 붙이며, 목적 문자열이 소비자끼리를 격리하고, 그 아래에서 키 링이 키를 댑니다."
category: "애플리케이션 보안"
related:
  - label: Key Ring
    slug: key-ring
...
- 이미 쓰고 있기 때문에 알아 둡니다. 쿠키 인증도 antiforgery 토큰도 TempData도 누가 설정했든 아니든 이 층을 지나 페이로드를 보호합니다. 그래서 레플리카 여러 대에서 인증 버그처럼 보이는 질문은 대개 이 API의 키에 대한 질문입니다. 우리가 설정하는 것은 그 키를 어디에 두고 어떻게 보호할지이고, 그 결정이 사는 자리는 Key Ring 페이지입니다.
...
- 이것은 장기 보관용 암호화가 아니고, 그렇게 여기면 데이터를 잃습니다. 만료된 키는 링에 남아 여전히 해제되지만, 프레임워크가 약속하는 것은 거기까지입니다. 폐기된 키, 지워진 키 파일, 잃어버린 링, 바뀐 애플리케이션 이름은 모두 작년 페이로드를 읽을 수 없게 만들고, 그중 어느 것도 버그가 아닙니다. 그런 일이 있고 나서도 해독할 수 있어야 하는 것은, 수명 주기를 우리가 쥐는 별도의 암호화 체계에 두어야 합니다.
...
- 목적 문자열은 계약이고, 바꾸면 기존 페이로드가 전부 무효가 됩니다. `"EmailConfirmation"`을 `"Email.Confirmation"`으로 이름만 바꿔도 사용자 메일함에 이미 들어간 링크가 전부 해제되지 않습니다. 문자열은 의식적으로 고르고, 인라인 리터럴이 아니라 상수로 두고, 하나를 바꾸는 일은 정리 작업이 아니라 마이그레이션으로 다룹니다.
- 인스턴스가 여럿이라면 위의 모든 이야기가 성립하기 전에 키 링부터 공유해야 합니다. 한 레플리카가 보호한 페이로드는 다른 링을 가진 레플리카에게는 의미 없는 바이트이고, 컨테이너의 기본값은 프로세스와 함께 죽는 링입니다. 지속화와 애플리케이션 이름을 설정하는 일이 위의 모든 것의 전제 조건이고, 그 방법은 Key Ring 페이지가 다룹니다.
...
public sealed class EmailConfirmationTokens(IDataProtectionProvider provider)
{
    ...
    public bool TryRead(string token, out Guid userId)
    {
        ...
        try
        {
            // 변조됐거나, 만료됐거나, 다른 purpose로 보호된 경우 모두 여기서 던집니다.
...
- `IPersistedDataProtector`는 무엇을 요청하는지 알고 있을 때만 씁니다. `ignoreRevocationErrors`를 넘긴 `DangerousUnprotect`는 *폐기된* 키로도 해제할 수 있게 해 줍니다. 만료된 키에는 특별한 처리가 필요 없습니다. 평범한 `Unprotect`가 여전히 받아 주기 때문입니다. 이것은 특정 마이그레이션을 위한 복구 도구이지 위의 주의점을 우회하는 길이 아니고, 이것을 습관처럼 꺼내고 있다면 그 페이로드는 애초에 짧게 사는 값이 아니었던 것입니다.
