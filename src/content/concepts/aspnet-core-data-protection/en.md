...
- This is not long-term encryption, and treating it as such loses data. Keys roll on a schedule and old ones are eventually removed, so a payload protected today may be unreadable in a year through no fault of yours: the framework owns that key lifecycle, not you. Anything that must still be decryptable after a rotation horizon belongs in a purpose-built encryption scheme with a key lifecycle you control.
...
- Use `IPersistedDataProtector` only when you understand what you are asking for. It allows unprotecting with a revoked or expired key, which is a recovery tool for a specific migration rather than a way around the caution above, and reaching for it routinely means the payload was never a transient one.
