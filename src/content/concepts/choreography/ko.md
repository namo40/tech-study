...
잃는 것은 흐름 자체입니다. 순서가 코드 어디에도 나타나지 않으므로 "12번 주문은 어디까지 갔고 왜 멈췄나"에 답하려면 서비스 세 곳의 로그를 읽어 사슬을 손으로 다시 이어 붙여야 합니다. 실패 처리도 이벤트가 흐르는 방향을 따라 흩어집니다. Inventory는 `ReservationFailed`를 발행할 뿐이므로, 그것을 구독하고 확보 실패가 환불을 뜻한다는 사실을 알고 있어야 하는 쪽은 Payment이고, Inventory의 실패 양상이 그것을 몰라도 될 서비스 안의 지식이 되어 버립니다. 순환이 생기기도 쉽고, 단계 순서를 바꾸는 일은 구독 여러 개를 한꺼번에 고치는 일이 됩니다.
