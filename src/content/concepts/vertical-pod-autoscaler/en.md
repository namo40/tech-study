...
The applying is the expensive half. For most of Kubernetes' history a pod's resources were immutable once it was admitted, so changing a request meant evicting the pod and letting the controller create a replacement with the new numbers. That is why the scene charges a restart for the correction: the capsules go down, come back, and come back the right size. In-place resizing has since arrived — stable in Kubernetes 1.35, and reachable from the autoscaler through VPA 1.4's `InPlaceOrRecreate` mode on clusters from 1.33 — and it removes the restart for many cases, but it is not universal: it depends on the resize policy, the resource and the container runtime, and the autoscaler falls back to recreating the pod whenever it cannot resize in place. Plan for the eviction and treat the in-place path as the improvement rather than the assumption.
