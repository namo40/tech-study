...
steps:
  - title: "One model for everything"
    text: "Writes that must obey rules and reads that need three joins share one code path and one database. The slow reads hold the tier for over a second each, so two commands end up standing outside a full box and come back late."
  - title: "Split the code, not the data yet"
    text: "Commands run the rules and return nothing but success. Queries return a row already shaped for the screen, read from a view. Still one database, but neither side waits on the other any more, and both meters stay low."
...
- Separate stores mean eventual consistency, so design the screen for it: show the command's own result, poll for the new one, or subscribe to it. What you must not do is read the query side straight after a write and treat the answer as authoritative.
...
Two handler interfaces and one `AsNoTracking()` query are the whole of the first step. The write side keeps change tracking, aggregates and validation; the read side never loads a domain object at all, because nothing on a screen needs one.
