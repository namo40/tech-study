---
title: "CQRS"
summary: "CQRS는 데이터를 바꾸는 모델과 데이터에 관한 질문에 답하는 모델을 나눕니다. 커맨드는 규칙을 거치고, 쿼리는 화면에 맞춰 만들어 둔 모양을 읽습니다. 저장소까지 나누는 것은 읽기와 쓰기의 확장이 정말로 갈라져야 할 때이며, 그때는 지연도 함께 받아들여야 합니다."
category: "애플리케이션 아키텍처"
scene: command-query-responsibility-segregation
steps:
  - title: "모델 하나로 전부"
    text: "규칙을 지켜야 하는 쓰기와 JOIN 세 번이 필요한 읽기가 같은 코드 경로와 같은 데이터베이스를 씁니다. 느린 읽기가 1초 넘게 계층을 붙잡고 있어서, 요청 셋(그중 둘은 커맨드)이 꽉 찬 박스 밖에 서 있다가 늦게 돌아옵니다."
  - title: "데이터는 아직, 코드부터 나눕니다"
    text: "커맨드는 규칙을 실행하고, 화면 분량의 데이터가 아니라 id 하나를 돌려줍니다. 쿼리는 화면에 맞춰 둔 행을 뷰에서 읽어 그대로 돌려줍니다. 데이터베이스는 여전히 하나지만 어느 쪽도 상대를 기다리지 않고, 두 미터 모두 낮게 유지됩니다."
  - title: "저장소를 나눕니다"
  ...
  - title: "read model은 버려도 됩니다"
    text: "모양을 바꾸고, 쓰기 쪽에서 다시 만들고, 쿼리를 거기에 다시 붙이면 됩니다. 비어 있는 동안 쿼리는 write store를 거치는 느린 길로 돌아갑니다. CQRS에 Event Sourcing이 꼭 필요한 것은 아니며, 읽기와 쓰기가 정말 다를 때만 나눕니다."
related:
  - label: Read Model
    slug: read-model
  - label: Projection
...
- 저장소를 나누면 최종 일관성(eventual consistency)이 따라옵니다. 화면을 거기에 맞춰 설계해야 합니다. 커맨드 자신이 만들어 낸 결과를 보여 주거나, 새 값을 폴링하거나, 구독하면 됩니다. 방금 쓴 것의 id와 버전을 돌려주는 커맨드는 패턴 위반이 아닙니다. CQRS가 금지하는 것은 커맨드가 화면 모양으로 다듬은 데이터를 건네는 일입니다. 쓰기 직후에 조회 쪽을 읽고 그 답을 확정된 값으로 취급하는 것만은 하지 말아야 합니다.
...
// 쓰기 쪽: 규칙을 거친 뒤 저장한다. 방금 쓴 것의 id를 돌려주고,
// 화면이 그릴 만한 것은 아무것도 돌려주지 않는다.
public sealed class PlaceOrderHandler(ShopDbContext db) : ICommandHandler<PlaceOrder, Guid>
{
    ...
    {
        var order = Order.Place(command.CustomerId, command.Lines);   // 도메인 규칙은 여기에 있다
        db.Orders.Add(order);
...
// 읽기 쪽: 뷰나 읽기 테이블에서 가져온 납작한 행. 추적도, 도메인 객체도 없다.
...
핸들러 인터페이스 두 개와 `Select`로 곧장 DTO를 만드는 쿼리 하나가 첫 단계의 전부입니다. 쓰기 쪽은 변경 추적과 aggregate와 검증을 그대로 가져가고, 읽기 쪽은 도메인 객체를 아예 만들지 않습니다. DTO로 프로젝션하면 추적할 엔티티가 남지 않기 때문입니다. 위의 `AsNoTracking()`은 비용이 없으니 습관으로 붙일 만하지만, 실제로 차이를 만드는 것은 엔티티 형식을 그대로 돌려주는 읽기 쿼리에서입니다.
