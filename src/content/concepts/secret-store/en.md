...
Then the list. A store that every workload in the cluster can read is a baked secret with extra infrastructure — the blast radius of one compromised process is again everything, and you have paid for a vault to get there. The value of a store is that access is a written-down decision per secret: this entry is readable by these workloads, and by nothing else. In Kubernetes the boundary is the namespace rather than the object: RBAC limits who may read a Secret through the API, not which pod in the namespace may mount it, and anyone who can create a pod in a namespace can read any Secret in it. In a cloud vault it is a role assignment on the vault or on the individual entry; with a CSI driver it is the pod's own identity that is presented at mount time, which is the shape that actually gets you a list per workload. All three are readable by a person during a review, which is the point.
