...
- 태그 기반 무효화는 논리적이고, 구독이 아니라 거친 도구입니다. `RemoveByTagAsync`는 그 태그의 차단 시각을 기록하고, 이후 읽기는 그보다 오래된 항목을 모두 미스로 취급합니다. L1에서도 L2에서도 아무것도 지워지지 않으므로, 그 항목들이 스스로 만료될 때까지 메모리는 회수되지 않습니다. "이 테넌트에 관한 전부" 같은 요구에는 맞는 도구이지만, 모든 노드가 같은 순간에 받는 이벤트처럼 기대하면 어긋납니다.
- L2로 넘어가는 것은 전부 직렬화되고, 그 비용도 캐시의 일부입니다. 미스마다 큰 객체 그래프를 직렬화하면 대체하려던 쿼리보다 느려질 수 있고, 큰 항목 하나가 공유 저장소에서 작은 항목 여럿을 밀어냅니다. 어쩌다 불러온 애그리게이트가 아니라 실제로 화면에 그리는 프로젝션을 캐시합니다. 크기에는 단단한 경계도 있습니다. `MaximumPayloadBytes`(기본 1 MB)를 넘는 값은 캐시되는 대신 로그만 남기고 건너뛰는데, "이 키는 왜 한 번도 히트가 안 나지"라는 질문의 흔한 답이 그것입니다.
...
- 패키지는 `Microsoft.Extensions.Caching.Hybrid`입니다. .NET 9와 함께 나왔지만 .NET 8과 .NET Framework 4.7.2에서도 돌아가므로 LTS 서비스도 쓸 수 있습니다. `AddHybridCache`가 적당한 기본값으로 서비스를 등록하고, `GetOrCreateAsync` 호출 하나가 조회와 미스 분기와 두 번의 쓰기를 대신합니다.
...
builder.Services.AddHybridCache(options =>
{
    ...
    {
        // 공유되는 L2 사본이 사는 시간입니다.
        Expiration = TimeSpan.FromMinutes(10),
        // 프로세스 안의 L1 사본은 둘 중 짧은 쪽이어야 합니다.
        LocalCacheExpiration = TimeSpan.FromMinutes(1),
    };
});

// 같은 키를 동시에 부르는 호출자들은 팩토리 실행 하나를 나눠 씁니다.
...
- 두 번째 층을 켜는 것은 `IDistributedCache` 등록입니다. 다른 것을 등록하지 않으면 HybridCache는 스탬피드 보호가 붙은 인프로세스 캐시입니다. `IDistributedCache` 구현을 아무거나, 예를 들어 `AddStackExchangeRedisCache`를 순서에 상관없이 등록하면 같은 호출부가 코드 한 줄 바꾸지 않고 공유 L2를 얻습니다.
