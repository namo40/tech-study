...
steps:
  ...
  - title: "지속 가능한 지점, 그다음 soak"
    text: "목표를 여전히 만족하는 가장 높은 부하로 물러납니다. 여기서는 p95 180ms에 초당 890건이고, 최고점인 1,060이 아니라 그 숫자가 용량입니다. 그다음 그대로 유지합니다. 힙이 40%에서 65%로 오르는데, 짧은 테스트로는 보이지 않는 누수입니다."
  - title: "현실로 만들기"
    text: "뜨거운 id 하나면 모든 요청이 캐시 히트가 되고, 같은 서비스가 초당 1,121건에 p95 24ms를 보고합니다. id 1만 개에 적중률 60%면 886건에 189ms입니다. 고정 도착률로도 잽니다. 얌전히 기다리는 사용자는 느린 서버가 자기 테스트를 늦추게 둡니다."
related:
  - label: Stress Test
    slug: stress-test
  - label: Spike Test
...
- 어떤 리소스가 먼저 포화되는지 찾기 위해 씁니다. 그래야 확장과 튜닝이 늘리기 쉬운 쪽이 아니라 진짜 병목을 겨냥합니다.
...
- 데이터 카디널리티, 페이로드 크기, keep-alive, HTTP 버전을 운영과 비슷하게 맞춥니다. 핫 키 하나면 모든 요청이 캐시 히트가 되고, 모든 숫자가 아무도 운영하지 않는 시스템을 놓고 하는 주장이 됩니다.
...
```csharp
// open 모델 시나리오입니다. 도착률 고정, 현실적인 id, 운영과 비슷한 클라이언트.
...
var ids = await File.ReadAllLinesAsync("order-ids.txt");   // 핫 키 하나가 아니라 실제 id 1만 개
...
var scenario = Scenario.Create("get_order", async ctx =>
    ...
    .WithLoadSimulations(
        ...
        Simulation.Inject(rate: 900, interval: TimeSpan.FromSeconds(1), during: TimeSpan.FromHours(2)));   // soak 테스트

NBomberRunner.RegisterScenarios(scenario).Run();
```

```text
# 도는 동안 클라이언트만이 아니라 서버도 지켜봅니다:
dotnet-counters monitor --process-id <pid> \
  --counters System.Runtime,Microsoft.AspNetCore.Hosting,Microsoft.Data.SqlClient.EventSource
```

`Simulation.Inject`이 open model입니다. 서비스가 어떻게 반응하든 정해진 속도로 계속 넣으므로, 느려짐이 조용히 줄어든 테스트가 아니라 늘어나는 큐와 오르는 지연으로 드러납니다. 위 시나리오가 담고 있는 세 가지 원칙은 k6를 포함해 어떤 도구를 쓰든 그대로입니다. 도착률을 open으로 유지하고, 핫 키 하나가 아니라 현실적인 id에 부하를 흩고, 운영과 비슷한 클라이언트로 몰아넣는 것입니다. 가장 자주 빠뜨리는 것은 넷째 원칙이고, 그것은 클라이언트 바깥에 있습니다. 클라이언트 쪽 수치만으로는 서버의 어떤 리소스가 포화됐는지 알 수 없으므로, 테스트가 도는 내내 서비스와 그 의존성의 카운터를 함께 모아야 합니다.
