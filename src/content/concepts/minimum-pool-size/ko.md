...
summary: "minimum pool size(최소 풀 크기)는 풀이 항상 데워 두는 바닥입니다. 아무리 놀고 있어도 닫지 않는 연결의 수이고, 콜드 스타트와 급증의 첫 파도에서 핸드셰이크 비용을 없애 줍니다. 그 대가는 데이터베이스가 대신 냅니다. 쓰지 않는 시간에도 그만큼의 연결이 계속 열려 있기 때문입니다."
