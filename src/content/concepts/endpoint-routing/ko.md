...
순서가 취향의 문제가 아니라 정해진 것인 이유도 여기에 있습니다. `UseRouting`은 엔드포인트 메타데이터를 읽는 모든 것보다 앞에 와야 하고, 엔드포인트 자체는 요청을 거절할 수 있는 모든 미들웨어가 차례를 마친 뒤 맨 끝에 와야 합니다. 이 순서를 어기면 대개 Authorization이 조용히 적용되지 않는 형태로 드러납니다.
