---
title: "Controllers"
summary: "Controllers는 ASP.NET Core의 클래스 기반 API 모델입니다. 액션을 타입 하나에 모으고, 같은 엔드포인트 라우팅 위에 애트리뷰트 라우팅을 얹고, 인증과 검증과 오류 처리를 되풀이되는 코드가 아니라 필터 파이프라인의 계층으로 만듭니다."
...
- 표면이 커져서 관례가 값을 하기 시작할 때 꺼냅니다. 여러 자원에 걸쳐 액션이 수십 개가 되면, 애트리뷰트 라우팅을 쓴 자원별 클래스가 모든 엔드포인트에 같은 모양을 줍니다. 새로 합류한 사람이 URL만 보고도 그것이 어디 있는지 짐작할 수 있게 됩니다. 짐작 가능성이 관례가 파는 물건이고, 엔드포인트가 많을수록 값이 올라갑니다.
...
- API 관례를 한 묶음으로 켜고 싶을 때 `[ApiController]`에 기댑니다. 애트리뷰트 라우팅이 필수가 되고, 매개변수 바인딩이 애트리뷰트 없이 출처를 추론하며, 모델 검증에 실패한 요청은 액션 본문이 돌기 전에 `400`으로 답을 받습니다.
...
- 액션마다 붙는 기계 장치가 있고, 핫패스에서는 그 값이 측정에 잡힙니다. 액션 선택과 모델 바인딩과 필터 호출과 결과 실행이 요청마다 각자의 비용을 치르므로, 부하가 큰 아주 작은 서비스라면 minimal-apis가 그 계층을 통째로 피합니다. 데이터베이스와 네트워크 시간이 대부분을 차지하는 평범한 API에서는 그 차이가 잡음 수준입니다.
- 액션이 마흔 개인 컨트롤러는 라우팅 애트리뷰트가 붙은 거대 클래스일 뿐입니다. 처방도 늘 하던 것과 같습니다. 동사가 아니라 자원으로 쪼개고, 로직은 액션이 부르는 서비스로 밀어내고, 컨트롤러는 HTTP와 도메인 사이의 얇은 번역으로 둡니다. 컨트롤러를 쓸모 있게 만드는 것도 읽기 어렵게 만드는 것도 같은 성장이고, 갈라놓는 사람이 있느냐가 갈림길입니다.
...
- `[ApiController]`와 애트리뷰트 라우팅이 표준 모양이고, 자동 검증 응답은 액션이 아니면 되풀이했을 일을 관례가 대신 해 주는 자리입니다.
...
- 필터는 그 필터의 범위가 있는 자리에 등록합니다. `builder.Services.AddControllers(o => o.Filters.Add<AuditFilter>())`는 모든 액션에 걸고, 클래스에 붙인 애트리뷰트는 컨트롤러 하나로 범위를 좁히며, `[ServiceFilter]`는 필터를 컨테이너에서 해석해 의존성을 받을 수 있게 합니다.
