...
steps:
  ...
  - title: "갈 때 스위치 하나, 올 때 스위치 하나"
    text: "트래픽이 한 번에 Green으로 넘어갑니다. 오류가 드러나면 되돌아가는 것도 즉시입니다. Blue가 아직 따뜻하게 살아 있기 때문입니다. 고쳐서 다시 넘깁니다. 블루-그린이 얻어 주는 것은 가장 값싼 롤백, 곧 아직 돌고 있는 옛 버전입니다."
  - title: "스위치 대신 다이얼"
    text: "카나리는 먼저 10%만 보내고 오류율을 지켜봅니다. 나쁜 버전이면 피해를 10%로 막은 채 되돌립니다. 좋은 버전이면 50%, 100%로 넓힙니다. 속도를 정하는 것은 달력이 아니라 지표입니다."
  - title: "Green이 없는 데이터베이스"
...
- 겹치는 구간 동안 환경 두 벌 비용을 냅니다. 그리고 그 구간은 전환 절차가 짧아 보이는 것과 달리 깁니다. 전환 전에 Green이 떠 있어야 하고 전환 후에도 Blue가 떠 있어야 합니다. 그러지 않으면 대가를 치르고 산 롤백이 존재하지 않습니다.
- 진짜 어려운 부분은 공유하는 데이터베이스입니다. 환경은 둘인데 스키마는 하나입니다. 모든 변경을 확장 후 축소로 나누고 그 사이에 릴리스를 하나 두지 않으면 롤백은 말뿐입니다. Blue로 돌아간다는 것은 Green이 쓴 데이터를 읽지 못하는 코드로 돌아간다는 뜻이기 때문입니다.
- 오래 붙어 있는 연결은 라우팅을 따라 움직이지 않습니다. WebSocket, SignalR 회로, gRPC 스트림은 버려진 쪽에 그대로 남습니다. 타임아웃이 끊어 주기를 기다리지 말고 직접 비우면서 클라이언트에게 다시 붙으라고 알립니다.
...
- 아무 일도 없는 날에 롤백을 예행합니다. 한 번도 써 본 적 없는 롤백 경로는 가설이고, 필요한 순간이 그것을 처음 테스트하는 순간이 되면 안 됩니다.
...
# 돌아가는 길도 같은 명령을 한 번 더 실행하는 것입니다
az webapp deployment slot swap --resource-group shop --name shop-api --slot staging
```

교체 직후 첫 요청이 느린 요청이 되지 않게 막아 주는 것이 예열입니다. Windows 플랜에서는 `web.config`의 `applicationInitialization`이 슬롯을 준비된 것으로 보기 전에 어떤 경로를 먼저 두드릴지 플랫폼에 알려 주고, Linux 플랜에서는(그리고 양쪽 모두에서 쓸 수 있는 대안으로) `WEBSITE_SWAP_WARMUP_PING_PATH`와 `WEBSITE_SWAP_WARMUP_PING_STATUSES` 앱 설정이 같은 말을 합니다. 어느 쪽이든 슬롯 고유 설정은 교체를 따라가면 안 되는 설정입니다.
...
규율이 필요한 자리는 데이터베이스이고, EF Core는 그 규율을 건너뛰어도 막아 주지 않습니다. 변경을 확장 마이그레이션과 축소 마이그레이션으로 나누고, 그 사이에 두 형태를 모두 쓰면서 새 형태를 읽는 릴리스를 하나 넣습니다. 확장 마이그레이션은 더하기만 합니다. 이름을 바꾸지도 않고 지우지도 않습니다.

```csharp
// 확장: 새 컬럼은 nullable로 도착하고, 기존 것은 아무것도 건드리지 않습니다.
migrationBuilder.AddColumn<string>(
    name: "Nickname", table: "Customers", type: "nvarchar(64)", nullable: true);

// 같은 릴리스에 실립니다. 둘 다 쓰고, 채워져 있는 쪽을 읽습니다.
customer.Nickname = value;
customer.Name = value;                       // 되돌아올 수 있는 버전을 위해 계속 씁니다
...
```csharp
// 축소: 롤백 창이 닫힌 뒤, 나중 릴리스에서 별도의 마이그레이션으로.
migrationBuilder.DropColumn(name: "Name", table: "Customers");
```

플랫폼이 교체 대신 가중치를 준다면, 예를 들어 Azure Front Door나 Traffic Manager나 서비스 메시라면, 같은 환경 두 벌이 카나리가 되고 스위치는 다이얼이 됩니다. 데이터 규칙은 하나도 바뀌지 않습니다. 바뀌는 것은 영향 범위의 크기뿐입니다.
