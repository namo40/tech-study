...
The corollary is that async is only real if it is real all the way down. An API that offers a method with `Async` in its name but performs a blocking call inside a `Task.Run` has not removed the wait, it has moved it to a different thread, and the pool pays for it either way. In .NET the difference is usually visible: on Windows a genuinely asynchronous file handle has to be opened with the right option, and a driver or a provider that does not support overlapped operation will fall back to a blocking call however the method is named. On Linux there is no asynchronous file I/O to ask for at all, so a file read there is a synchronous call scheduled on the pool whatever option you passed, while sockets are genuinely asynchronous on both.
