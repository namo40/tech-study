...
Sending a query to the replica is usually a connection string rather than code. On SQL Server, an availability group listener reads `ApplicationIntent=ReadOnly` and routes that connection to a readable secondary, provided read-only routing is configured on the group; on PostgreSQL and MySQL you point a second connection at the replica's host and open it read-only. In an application that is a second context or a second session factory over the same model, resolved per operation, and the useful discipline is to make the read-only one genuinely read-only, so that a stray `SaveChanges` fails loudly instead of failing at the database.
