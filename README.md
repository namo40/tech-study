...
**Site:** https://namo40.github.io/tech-study/
...
Scenes start playing when the page loads. Choosing a step, with the step buttons, the previous and next buttons, or the arrow keys, jumps to the start of that step and pauses there so the diagram can be studied; play continues from that point. When the reader prefers reduced motion, the player waits on the first frame instead of starting on its own. The current step is shown as a caption card beside the stage on a wide screen and above it on a narrow one, so the explanation and the diagram stay together without either covering the other. Captions are page text rather than drawings, states are labelled as well as coloured, and results are marked with a check or a cross so colour is never the only signal. The text size button in the header steps the body text and the captions through five sizes and remembers the choice, while the drawing on the stage keeps the size it was designed at.
